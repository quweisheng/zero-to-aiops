...
- For text diagrams or knowledge maps that use `->`, do not explain the arrow as a flow arrow. Explain the English words or acronyms on that line, and explain Chinese-only lines as the actual subtopic or operation the learner should understand.
- For plain text examples, explain concrete English fields and values instead of using generic wording. For example, `service=order-api` means `service` is the service-name field and `order-api` is the concrete service name; `severity=critical` means `severity` is the alert severity field and `critical` is a high-priority severity value. Chinese-only text lines do not need extra line-by-line explanation.
