...
- For examples, use concise inline comments, nearby prose, or a small focused field table only when it improves readability. Do not add generated line-by-line explanation tables unless the user explicitly asks for them.
- For text diagrams or knowledge maps that use `->`, explain important English words or acronyms in surrounding prose when needed, but keep the diagram itself clean.
