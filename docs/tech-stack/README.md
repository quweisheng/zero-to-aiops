...
2026 年 9 月的全站老师带学修订已覆盖全部 114 篇技术栈：用零基础能理解的前置概念、中文注释知识地图、机制推理、两级实验和连续面试追问组织课程。每篇均达到不少于一万字中文正文，较长文章保留完整内容；代码、输出和文字图不计入这一目标。逐篇实际篇幅与复核方向见 [老师带学修订与篇幅复核](./teaching-coverage.md)。

第一次学习，请从 [零基础路线](../roadmap/README.md) 选一条业务链，跟做后合上文章独立复现，再用 [面试训练](../interview/README.md) 检验能否解释。每篇的“预期结果”是练习判断标准，自己的实际运行记录要单独保存。

每篇按 [技术栈精讲写作标准](./writing-standard.md) 组织官方资料、场景开场、是什么、原理、架构、配置、实验、排障、面试题和学习证据，后续维护继续沿用。课程同时覆盖零基础练习和进阶面试：内部链路、生产高可用、容量、安全、升级、故障场景和连续追问。达到篇幅目标不是录用保证；编码、基础知识、真实项目和沟通仍需独立练习。
...
| 数据与 AI | MySQL/SQL、Oracle、PostgreSQL、MongoDB、Redis、Kafka、ActiveMQ、RabbitMQ、ZooKeeper、Hadoop、Hive、HBase、Spark、Flink、HAWQ 存量迁移、pandas、机器学习、scikit-learn、TensorFlow、PyTorch、Transformer、Ollama、FastAPI、LLM/OpenAI API、LangChain、LangGraph、Dify、RAG、向量数据库 | P1-P3 | 能处理关系与文档数据、消息队列、事件流、分布式协调、湖仓 SQL、随机读写、批流计算、遗留平台迁移、本地模型推理和 AI 应用，做异常检测、告警降噪与智能运维助手 |
...
7. [Java](./foundation/java.md)：从语言与 JDK 工具链深入字节码、类加载、JIT、GC、并发、JFR 和企业生产排障。
8. [RESTful API](./foundation/restful-api.md)：理解资源、HTTP 语义、契约、幂等、并发控制、安全和可观测性。
9. [Kubernetes](./cloud-native/kubernetes.md)：从对象操作深入 API、控制器、调度、网络、存储、高可用和生产排障。
10. [etcd](./cloud-native/etcd.md)：理解 Kubernetes 状态存储、Raft 多数派、MVCC、Watch、备份和恢复。
11. [Calico](./cloud-native/calico.md)：理解 CNI、IPAM、BGP/封装、NetworkPolicy 和网络故障链路。
12. [Cilium](./cloud-native/cilium.md)：理解 eBPF 数据面、身份策略、Service 负载均衡和 Hubble 可观测性。
13. [Traefik](./cloud-native/traefik.md)：理解动态 Provider、入口路由、Gateway API、TLS、可观测性和 4xx/5xx 证据链。
14. [KVM 虚拟化](./virtualization-private-cloud/kvm.md)：理解 KVM、QEMU、libvirt、Virtio、虚拟网络与存储、迁移、高可用和平台排障。
15. [VMware vSphere](./virtualization-private-cloud/vsphere.md)：理解虚拟机如何共享物理资源，以及集群、迁移、网络、存储和平台排障。
16. [IBM Storage](./storage-data-protection/ibm-storage.md)：理解业务数据最终如何落盘、保护、监控和恢复。
17. [Dell EMC VPLEX](./storage-data-protection/dell-emc-vplex.md)：理解异构块存储虚拟化、Metro 同步双活、Consistency Group、Detach Rule 和 Witness 仲裁。
18. [Dell EMC VMAX](./storage-data-protection/dell-emc-vmax.md)：理解高端阵列 Engine/Director、SRP/TDEV、Masking View、SnapVX、SRDF 和容量性能排障。
19. [Brocade 6510 光纤交换机](./storage-data-protection/brocade-6510.md)：理解 FC 登录、zoning、FSPF、BB Credit、慢排水、双 Fabric 和 EOS 迁移。
20. [华为 OceanStor](./storage-data-protection/huawei-oceanstor.md)：理解企业阵列的数据路径、LUN 映射、多路径、双活、复制和存储排障。
21. [爱数 AnyStorage](./storage-data-protection/aishu-anystorage.md)：理解统一 SAN/NAS、RAID 2.0、GX 虚拟化、双活、复制和存储 AIOps 排障。
22. [Ceph](./storage-data-protection/ceph.md)：理解对象如何经过 pool、PG 和 CRUSH 分布到 OSD，并学会判断集群健康。
23. [IBM WebSphere](./middleware-application-platform/websphere.md)：理解 traditional ND 与 Liberty 边界、企业 Java 请求路径、EAR 更新、BLA/Composition Unit、集群、会话、JDBC/JMS/JTA 和生产排障。
24. [Apache Tomcat](./middleware-application-platform/tomcat.md)：理解 Servlet 容器、请求与部署路径、线程/连接/JDBC、会话、可观测性和生产升级回滚。
25. [GitHub Actions](./automation/github-actions.md)：把重复动作变成流程。
26. [Apache Maven](./automation/maven.md)：理解 POM、生命周期、依赖解析、插件、多模块、企业制品库和可复现构建。
27. [SonarQube](./automation/sonarqube.md)：把静态分析、质量规则和 Quality Gate 接入 CI/CD 与变更风险闭环。
28. [MongoDB](./data-ai/mongodb.md)：理解文档模型、索引、WiredTiger、复制集、分片和 Change Stream。
29. [Apache ActiveMQ](./data-ai/activemq.md)：理解 Classic 与 Artemis、JMS、持久化、确认、死信和高可用边界。
30. [pandas](./data-ai/pandas.md)：用数据做异常检测、告警降噪、智能助手。
31. [机器学习](./data-ai/machine-learning.md)：理解样本、特征、标签、评估和异常检测边界。
32. [TensorFlow](./data-ai/tensorflow.md)：把机器学习主线落到张量计算、模型训练、分布式性能和线上交付。
33. [PyTorch](./data-ai/pytorch.md)：理解 Tensor、Autograd、训练状态、性能、分布式扩展、模型交付和 AIOps 故障排查。
34. [Kubeflow](./cloud-native/kubeflow.md)：把 Kubernetes、流水线、超参数搜索、分布式训练、模型元数据和推理交付连成可治理的机器学习平台。
35. [Transformer](./data-ai/transformer.md)：理解 Token、Q/K/V、自注意力、Encoder/Decoder、KV Cache、长上下文容量和模型生产故障。
36. [Ollama](./data-ai/ollama.md)：理解本地模型拉取、加载、推理、流式 API、显存容量、安全边界和 AIOps 服务化。
37. [LangChain](./data-ai/langchain.md)：理解 v1 Agent Harness、Tool Loop、Middleware、状态恢复、RAG/MCP、生产安全与 AIOps 证据助手。
38. [Dify](./data-ai/dify.md)：把模型、知识、工具和工作流组装成可发布、可观察、受控的 AIOps 应用。
39. [网络安全等级保护（等保 2.0）](./security-compliance/mlps.md)：把资产、控制、证据、整改和持续监控连成闭环。
40. [JumpServer](./security-compliance/jumpserver.md)：把自然人、资产账号、最小授权、连接代理、录像与命令审计连成 PAM 闭环。
41. [IT 项目管理 / PMP](./architecture-delivery/it-project-management-pmp.md)：学习立项、WBS、关键路径、EVM、风险、变更、敏捷与 DevOps/SRE 项目治理。
42. [Kubernetes 平台运维岗位专项路线](../interview/kubernetes-platform-operations-role.md)：按招聘要求串联平台、交付、IaaS 和方案证据。
43. [Apache ZooKeeper](./data-ai/zookeeper.md)：理解分布式协调、会话、Watch、选主、多数派和上层依赖故障。
44. [Apache Hadoop](./data-ai/hadoop.md)：理解 HDFS、YARN、MapReduce、HA、容量、安全、升级和数据平台排障。
45. [Apache Hive](./data-ai/hive.md)：理解 HiveServer2、Metastore、SQL 编译执行、分区、文件格式、ACID 和数据仓库排障。
46. [Apache HBase](./data-ai/hbase.md)：理解 RowKey、Region、WAL、MemStore、HFile、Compaction、热点和低延迟随机读写。
47. [Apache Spark](./data-ai/spark.md)：理解 Driver/Executor、DAG、Stage/Task、Shuffle、SQL、Streaming 和性能排障。
48. [Apache Flink](./data-ai/flink.md)：理解 Event Time、Watermark、State、Checkpoint、反压和端到端一致性。
...
- [IT 项目管理 / PMP](./architecture-delivery/it-project-management-pmp.md)
- [FDE 岗位技术栈与实战路线](../interview/forward-deployed-engineer-role.md)
