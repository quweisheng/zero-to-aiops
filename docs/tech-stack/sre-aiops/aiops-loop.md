...
AIOps 不是单个工具，而是一条闭环：采集数据、发现异常、关联上下文、定位原因、推荐动作、执行 runbook、验证效果、沉淀知识。学闭环能防止只做“模型 demo”，却无法落到运维流程。

## 它解决什么问题

- 把指标、日志、链路、告警、变更串成处理流程。
- 明确 AI 在分析、摘要、推荐中的位置。
- 把自动化动作限制在可验证、可回滚的边界内。
- 让每次故障处理反哺知识库。
- 帮助设计可展示的 AIOps 项目作品。
...
- [ ] 我能画出 AIOps 从数据采集到自动化处理的闭环。
- [ ] 我能说明每一层需要哪些技术栈。
- [ ] 我能区分异常检测、告警降噪、根因分析、自动化修复。
- [ ] 我能说明 LLM 在闭环中的安全边界。
- [ ] 我能设计一个最小 AIOps 项目切片。
- [ ] 我能把故障处理结果沉淀回知识库。
...
1. AIOps 闭环包含哪些阶段？
2. 为什么 AIOps 不能只等于机器学习模型？
3. 指标、日志、链路、变更分别提供什么信息？
4. 告警降噪和根因分析有什么区别？
5. 自动化修复为什么需要审批、验证和回滚？
6. LLM 在 AIOps 中适合做哪些工作？
7. 如何设计一个从 0 到 1 的 AIOps 项目？
8. 如何证明 AIOps 项目真的改善了运维效率？
