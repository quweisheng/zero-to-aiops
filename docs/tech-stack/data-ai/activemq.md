...
初学者先补三个前置：进程是正在运行的程序，端口是它接收连接的编号，持久化是让状态在进程退出后仍有受支持的恢复路径。只需阅读 [Java](../foundation/java.md) 的运行环境部分与 [网络基础](../foundation/networking.md) 的地址、端口部分；本课不要求先搭 Kubernetes。先做五条消息实验，再回来解释确认和存储，最后推演消费者停机。
...
图里 Queue 是队列，Durable Subscription 是服务端保存身份和进度的持久订阅；Ack 是确认，Transaction 是事务，Redelivery 是再次投递。Ordering 指顺序约束，Duplicate 指重复，JMX 是 Java 管理扩展，Web Console 是网页控制台。图中箭头表示学习依赖，不意味着每条消息都会依次经过所有协议。后文 Session 是会话上下文，Dispatch 是消息分发，Commit 是提交，Primary 是主节点而非数据库主键。
...
假设告警入口生成事件甲，等待发送响应时网络断开。你有两种可能：请求根本没有抵达 Broker；请求已经持久保存，只是响应没有回来。客户端抛出同一个超时异常，不能区分这两种历史。这里的“未知”是证据不足，不是失败的另一种说法。

如果立即生成新业务编号再发送，同一告警可能建立两张工单。正确的恢复协议要沿用原业务编号，记录每次尝试，查下游已有结果，或者依赖持久化幂等记录允许安全重试。Broker 分配的消息编号适合定位一次投递，不能替代业务系统对“同一件事”的定义。

我们把账分成三本：发件箱保存待发布的业务事件，Broker 保存可投递的消息，消费者数据库保存业务结果。它们不是一个共同事务。发件箱把事件和原业务更新写入同一数据库事务，只解决“数据库已改却忘了发消息”的窗口；投递程序仍可能重发，消费者仍要幂等。

幂等不是在业务写入前查一次是否存在。两个消费者同时查询都发现不存在，随后都写入，仍会重复。应由唯一约束、受控状态迁移或下游幂等接口完成并发裁决。重复请求还要比较请求内容，防止同一个业务键被误用于两个不同操作。

现在合上答案：如果幂等记录写成功而工单写失败，应该重投还是跳过？参考答案是先查看两者是否同事务。若不在同事务，单凭“已处理标记”跳过会漏建工单，需要可恢复的中间状态或对账。选择数据库事务边界，比给每条日志加一个成功字样更重要。

### 用纸面状态表重现确认丢失

这是生产语义模拟，不运行 Broker。准备纸张或电子笔记，建四列：事件编号、工单行数、确认是否送达、Broker 是否仍可能重投。初始事件甲对应零行、未确认、可投递。第一步模拟数据库事务提交，把行数改成一；第二步在确认发送前模拟消费者退出，保持确认未送达。

第三步把同一事件再交给消费者。没有唯一约束时，行数从一变二；有原子幂等约束时，查询并返回原结果，行数仍为一。第四步让确认成功，标记本次投递完成。预期两种实现都可能收两次消息，但只有幂等实现保留一份有效业务结果。这是本模型唯一要证明的差别。

验证时数的是业务行，不是消费函数调用次数。再做反例：先把确认标为送达，再模拟数据库写失败，消息可能不再重投，行数为零。将两张表并排比较，写出你选择“提交业务后确认”的原因。清理只需删除这几行合成记录，没有服务器、队列或目录需要删除。

若推演出现“既没有重复也不会漏”，检查你是否把两个独立系统错误画成一次原子操作。真实 XA 是跨资源协调事务的另一类方案，有协调器、资源兼容和恢复成本；本课没有实施 XA，也不把纸面模拟称为实际事务故障验证。

### 顺序需求怎样转成业务设计

设备先产生“告警打开”，再产生“告警恢复”，恢复处理先完成会造成界面重新打开旧告警。要求整个队列只有一个消费者可以减少并行乱序，却让所有设备共享一个串行瓶颈。更合理的问题是：必须排序的是全部告警，还是同一设备的同一事件生命周期？

Classic 的消息分组用 `JMSXGroupID` 表达相关消息的分发归属，可以让不同组并行；但组的拥有者会变化，组映射容量、预取和重投也要核对。它并不授权消费者把同组任务再次交给无序线程池。观察时同时记录组键、消费者身份与业务版本，不能只看发送序号。[官方消息分组说明](https://activemq.apache.org/components/classic/documentation/message-groups)。

即使投递保持所需顺序，外部接口超时仍可能让旧操作晚完成。业务对象可维护单调版本号，以条件更新拒绝较旧状态覆盖较新状态。这个版本判断与重复判断不同：重复表示同一操作已执行，陈旧表示操作合法但已经被更新的状态取代，两者都需要留下原因而非静默吞掉。

### 用数字判断积压恢复是否可行

每秒进入二千条，正常完成二千五百条，消费者停十分钟就积压一百二十万条。恢复后净减少只有每秒五百条，理想清空时间是二千四百秒，也就是四十分钟。用总消费速度相除得到八分钟，漏掉了持续到来的新消息。

每条有效载荷二 KiB，这些积压约为二点二九 GiB，但不是磁盘最终占用。消息头、日志段、索引、未及时清理的段、其他队列和备份都要另算。删除或确认消息也不保证文件系统立即归还同等空间，因此容量告警应结合磁盘实际余量与 Store 用量变化。

再假设新版本每条消息调用外部服务五次，数据库仅允许每秒六千次请求。理论业务上限只有每秒一千二百条，已经低于入口。继续加消费者不能改变这个上限，反而让连接池等待和重投更严重。修复路径是减少重复查询、恢复旧版本或限制入口，而不是让消息服务器承受无限积压。

流控让生产端阻塞或失败，是保护有限存储的机制，不应一看到发送慢就关闭。应观察目的地与系统级高水位、生产发送时延和消费者完成率，明确阻塞多久转入受控失败。异步发送还有客户端缓存边界，返回快不意味着 Broker 已接收全部数据。[官方流控说明](https://activemq.apache.org/components/classic/documentation/producer-flow-control)。

### 从事故证据到自动化动作

AIOps 可以将队列年龄、净增长速率、消费者版本和数据库等待关联起来，给出“新版本处理成本增加”的候选解释。它不能因为时间接近就宣布根因。用同类消费者的不同版本对照、受控回退后的吞吐变化与调用次数，才逐步增强证据。

自动扩容应同时检查下游余量和有序组分布，设并发上限与停止条件。自动重放死信则还需额外核对错误已修复、旧结构可读取、业务键仍有效、重放速率和审批范围。复制死信消息到主队列是一项新的发送，不能假定消费者仅凭新的消息编号识别原业务。

交付给值班人的三分钟答案应落到一条完整推理：先确认是哪条产品线，再追生产确认、存储、分发和业务提交；根据队列年龄与下游证据定位瓶颈；选择有界限流、回退或扩容；最后验证业务唯一性、旧积压、新消息时延与存储冗余。只展示端口恢复或队列归零，无法证明事件确实处理正确。
