...
Redis 常用于缓存、计数器、排行榜、分布式锁、队列和临时状态。AIOps 项目里，它适合保存短期告警状态、去重窗口、任务队列、会话缓存和热点查询结果。
...
- 保存告警去重窗口和临时状态。
- 支持简单队列、发布订阅和限流计数。
- 作为 Python/FastAPI AIOps 服务的辅助存储。
...
- [ ] 我能解释 Redis 为什么快。
- [ ] 我能使用 string、hash、list、set、sorted set。
- [ ] 我能设置 key 过期时间。
- [ ] 我能说明缓存穿透、击穿、雪崩的区别。
- [ ] 我能设计一个告警去重 key。
- [ ] 我能说明 Redis 在 AIOps 服务中的适用边界。

## 面试题

1. Redis 适合存什么数据？
2. Redis 和 MySQL 的主要区别是什么？
3. Redis 常见数据结构有哪些？
4. TTL 在告警去重里有什么用？
5. 缓存穿透、击穿、雪崩分别是什么？
6. 什么场景不适合用 Redis？
7. 如何用 Redis 做简单限流？
8. Redis 持久化和内存淘汰为什么重要？
