...
先修只要求会读一张表、理解平均值与比例、能运行 Python。矩阵可以先当作多行多列的数字表，向量就是一行数字；梯度先理解为告诉参数朝哪个方向调整的变化率。你不必先学完高等数学才能开始，但应能手算误报与漏报的分母，知道时间先后不能颠倒。
...
这里依次是请求速率、错误比例、延迟、是否发生发布和重启次数；输出是事故候选或噪声候选。`high priority`、`medium priority`、`likely noise` 分别指高优先级、中优先级与可能噪声，不是已经被人工确认的标签。后面图中的 `deployments` 是发布记录，`enrichment` 是给告警补充信息。
...
如果 PowerShell 不允许激活虚拟环境，可以不修改执行策略，直接使用课堂环境里的解释器：

```powershell
.\.venv\Scripts\python.exe -m pip install pandas numpy scikit-learn
.\.venv\Scripts\python.exe ml_alert_classifier.py
```
...
分钟窗口级别召回率与事故级别召回率并不相同。一场持续六十分钟的事故被检测到五十九个窗口，看起来很优秀，但另一场只有一分钟的严重故障完全漏掉。窗口统计偏向长事件，事故统计则把每次独立事件作为单位；同时报告两种结果，才能解释模型究竟在哪种意义上有效。

提醒次数也不能直接当作发现次数。同一事故连续提醒二十次，值班成本是二十次打扰，事故覆盖仍只有一次。去重与告警合并规则会改变业务效果，应把模型原始分数、阈值判断和通知策略分别评估。否则你可能把通知策略的改善误认成模型算法的提升。

再看低基率。假设十万个窗口里只有一百个事故窗口，模型抓住其中九十个，召回率是百分之九十。若剩余九万九千九百个正常窗口中有百分之一误报，就产生九百九十九个误报；精确率是九十除以一千零八十九，约百分之八点三。看似很低的误报率，在稀有事件上仍会造成大量无效提醒。

因此设计阈值时，应画出“每班提醒量与事故覆盖”的对应关系，并分关键服务、时段和严重程度检查。不能只追求一条总体曲线下面积。ROC 曲线观察不同阈值下的真阳性率和假阳性率；精确率与召回率曲线更直接展示被提醒样本中的价值，但两者都离不开真实的标签和采样分布。

如果要求每班最多五十条候选，可以先比较各阈值的提醒预算，再审查被舍弃的严重事故。预算是资源限制，不是允许漏掉任何事件的理由；有些强规则必须始终保留，模型仅为剩余候选排序。还需要明确预算超出时是降级、升级值班支援，还是延后低风险复核。

## 线上特征为什么和离线表格不一样

离线训练通常能看到迟到补齐的指标，而在线决策可能只能看到已经到达的数据。若训练把完整五分钟平均值当输入，线上却只有前三分钟，模型面对的就不是同一个问题。每个特征应明确事件时间、到达时间、观察截止时间、缺失处理和成熟条件，保证训练时模拟当时真正可见的材料。

同样一条百分位延迟，直接平均多个实例的百分位数一般不能得到全服务百分位数。特征构造要先核对监控口径，必要时从可合并的直方图桶或原始分布重新计算。模型不会替你修正统计上错误的输入，更多训练数据只会让它更稳定地学习错误口径。

冷启动也是一种状态。新服务没有足够历史窗口，不能把缺失历史填成全零后默默按成熟服务评分。可以标为数据不足、使用规则兜底或选择专门冷启动策略。记录“为何没有给分”同样有价值，它能区分正常低风险和模型根本没有可信输入。

一个实用排障顺序是固定同一条脱敏输入，分别在离线流水线与线上流水线转换，对比最终特征列名、顺序、数值、缺失标记和版本。若两边不同，先修输入合同；若相同再比较模型文件、依赖、阈值和后处理。不要一看到预测不同就立即重训。

## 模型发布物也是需要防护的软件

模型文件并非天然安全的数据。有些 Python 持久化格式在加载时能够执行代码，因此不能从陌生地址下载一个所谓异常模型就直接加载到有生产权限的进程。使用可信制品来源、校验完整性、限定加载权限，并对依赖和序列化格式进行审查。参考 [scikit-learn 模型持久化说明](https://scikit-learn.org/stable/model_persistence.html)。

模型加载成功也不证明跨版本结果兼容。发布时记录训练依赖和推理依赖，在目标环境用固定样本比较类别、概率、阈值分流和性能。升级库后反序列化旧模型不是理所当然受支持的迁移路径；必要时使用旧环境服务旧模型，或重新训练并走评估发布。

推理服务要避免资源层层放大。一个请求处理进程内部可能启动多个数值计算线程，多进程再乘多线程会造成过度争抢。先测单实例的吞吐、尾延迟和内存，再确定进程与线程预算。批量预测提高吞吐时，也要计入等凑一批的等待时间，不能只展示计算阶段的快。

模型不可用时，系统应输出明确的不可用或降级标记，保留原始告警与规则结果。不能把异常分数默认填零并当作正常，也不能用上一个服务的预测顶替。高可用验收包含依赖加载失败、特征服务不可用、队列积压和版本不一致，而不仅是进程健康检查返回成功。

## 三分钟完整回答示范

我会先把 AIOps 任务限定为“在某个服务窗口结束时，对告警是否值得优先复核进行评分”，不是让模型直接自动修复。每个样本只包含当时可见的指标、发布和历史信息，标签来自成熟后的人工事故记录。先明确误报预算与关键故障召回，再选择一个能解释和复现的简单基线。

训练时把预处理和模型放在同一流水线内，只使用训练部分拟合；按时间与事故分组，避免连续窗口跨集合泄漏。先对比规则基线，再看混淆矩阵、精确率、召回率、事件覆盖和提醒次数。阈值在验证集选择，最终测试集不反复用于调整，标签迟到的样本单独标记。

上线时把特征定义、预处理、模型和阈值绑定版本，先影子运行再灰度，仅补充告警证据或排序。监控输入缺失、单位变化、分布、分数和推理耗时，再用成熟标签评估业务效果。发现异常先对照固定输入定位数据、模型还是决策层问题，保留规则兜底和已验证的回退包。

如果面试官追问“为什么不用准确率证明收益”，我会用稀有事故的例子说明全猜正常也可能高分，且窗口级高分会掩盖独立短事故。最终要证明的是减少多少无效打扰、覆盖多少重要事故、有没有新增风险，而不是把一张合成数据的满分报告包装成生产收益。

最后给学习者一个独立作业：设计一条“采集延迟导致特征不完整”的事故记录，写出三种竞争假设、每种需要的证据和如何证伪。只允许用脱敏或合成样本，不能写不存在的业务收益。基础实验退出后没有后台资源；保留脚本、依赖与输出，清理仅涉及自己新建的课堂目录内容。
