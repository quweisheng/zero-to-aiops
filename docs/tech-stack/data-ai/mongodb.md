...
本文实验固定使用 `mongo:8.2.12`，它是保留的课堂基线，不是对当前生产版本的推荐。上面的版本信息是注明日期的历史背景，不代表当前最新状态。移动标签可能改变，因此应固定完整补丁版本，并在自己的验证记录里保存镜像 Digest；部署前另查安全补丁与支持边界。
...
零基础先把对象、数组、字符串、数字和日期类型分清，再理解客户端与服务器是不同进程。地图中的 `Database` 是数据库，`Embedded` 是嵌入子文档，`Reference` 是保存关联对象的引用。复制图里的 Primary 是主节点、Secondary 是复制成员，不能把“从节点”理解成永远不能成为主节点。
...
AIOps 平台每天接收主机、应用、告警、工单和变更事件。不同来源的字段不完全一样。下面每行分别展示一个 JSON 对象，不是一个整体 JSON 请求体：
...
应用
  ...
  -> WiredTiger 并发控制和缓存
  -> 文档修改与 Oplog 记录协调提交
  -> Journal 崩溃恢复与后续 Checkpoint 持久化
...
上图是职责路径，不是每条写入都等待一次完整 Checkpoint 的串行时间表。日志刷写、复制与后台检查点有各自调度；具体确认边界由写关注及相关配置决定。
...
应用不应把固定 Primary 地址当作永远不变的写入口。支持复制集发现的驱动从可用种子发现完整拓扑；一个种子可用于发现，但若它启动时不可达就缺少其他入口，因此应配置多个 Seed 或使用受支持的服务发现方式。
...
这份配置是待按环境补齐证书、目录权限和成员认证的说明片段，不是可直接部署的生产成品。MongoDB 六点一以后已移除 `storage.journal.enabled`，本课版本始终启用日志，不要从旧教程复制这个已删除的开关。
...
```powershell
docker compose -f compose.rs.yaml -p mongodb-rs-lab exec mongo1 mongosh `
  "mongodb://mongo1:27017,mongo2:27017,mongo3:27017/?replicaSet=rs0" --quiet --eval @'
db = db.getSiblingDB("aiops");
db.events.insertOne(
...
开始前创建独立课堂目录，将单节点文件保存为 `compose.yaml`，初始化文件保存为 `init/01-user.js`；复制集文件另存为 `compose.rs.yaml`。检查两个课堂项目名及相关卷没有被已有工作占用，并核对端口空闲。发现同名资源就停下改名，不通过删除旧卷强行让初始化脚本重跑。

基础实验需要 Docker 可运行 Linux 容器，以及足够容纳数据库进程的内存和磁盘。文中的明文密码只用于绑定本机回环地址的合成课堂，不能复制为生产凭据。连接字符串也可能被进程列表或终端记录看到，真实环境应使用适合的凭证管理方式，不把它们放入截图。

初始化只作用于新空卷，因此记录“脚本执行过”要有集合验证器、用户权限和索引定义的查询结果。启动健康检查只证明能响应连接，不证明初始化对象都正确。一个错误类型文档被拒绝，才是结构验证实际生效的证据；合法文档能写入，则证明没有把所有业务一概挡住。

TTL 是后台过期删除机制，不是精确到秒的业务时钟。当前实验设置七天保留，刚插入的事件不会马上消失。要观察过期行为，可以在这个专用教学集合插入一条日期为八天前的合法事件，再反复查询它是否仍存在；删除时间受后台扫描与负载影响，不承诺固定几秒内完成。

```javascript
const expiredId = "lesson-expired-only";
if (db.events.findOne({_id: expiredId})) {
  throw new Error("课堂 ID 已存在，请改用新 ID，避免覆盖旧数据");
}
db.events.insertOne({
  _id: expiredId, type: "alert", service: "ttl-lab", severity: "info",
  createdAt: new Date(Date.now() - 8 * 24 * 60 * 60 * 1000)
});
db.events.findOne({_id: expiredId});
// 等待后重新查询上一行；后台删除后应返回 null
```

若仍存在，先检查时间字段确为日期、TTL 索引存在以及后台删除是否有积压，不要反复插入更多旧数据。结束时用这个精确 `_id` 删除仍残留的课堂记录，或在已确认拥有的课堂项目清理时一起删除。业务若必须隐藏已过期数据，应在查询条件里限制时间，不能等待后台物理删除代替权限或有效期判断。

复制集实验是在一台机器上停止一个数据库进程，并不能证明能抵御宿主机断电、机房网络分区或磁盘损坏。三容器共享同一宿主机仍属于同一个物理故障域。报告中写清停止对象、失效时间、新主出现时间和原记录是否可读，不把它宣传为跨区域灾备实测。

## 单文档竞争之后，还要问执行发生了几次

版本条件领取保护的是这次文档状态变更，不能自动保护领取以后调用的远端重启接口。进程领取成功、执行动作后崩溃，恢复时仍需要判断动作是否发生。将稳定操作编号、外部结果查询和业务状态纳入设计，不能因为领取原子就声称整条自动化精确一次。

多文档事务提交也有结果未知的时刻。客户端没有拿到提交确认时，应按驱动的事务错误标签与重试规则处理，不凭一个普通网络异常自行重做所有外部效果。事务回调中发邮件或调用运维接口尤其危险，因为回调可能重复执行，而这些副作用不在数据库事务范围内。

Change Stream 的恢复令牌是消费进度，不是无限有效的备份坐标。先处理业务再保存令牌，故障恢复可能重复事件；先保存令牌再处理业务，故障可能漏掉效果。可把下游幂等结果与进度共同管理，并准备历史不足时的全量对账与增量衔接方案。参考 [官方变更流说明](https://www.mongodb.com/docs/manual/changeStreams/)。

当消费者要求更新后的完整文档，还应分清事件描述与查询得到的当前文档。在变化很快时，后续查到的对象可能已包含更晚更新；不要不加核对地把当前对象当作该事件当时的精确快照。若业务依赖前后版本，要核实目标版本支持的前后镜像能力、保留条件与成本。

## 容量题要同时算数据、索引和恢复速度

设计题的每秒两万写是峰值，不应直接假设全天持续；先采集平均与峰值持续时间。若为了压力预算假设全天两万条、每条平均一千字节，一天就是十七点二八亿条、约一点七二八太字节原始文档，九十天约一百五十五点五二太字节。三个完整副本仅原始数据就约四百六十六点五六太字节。

这个估算尚未计索引、Oplog、日志、临时空间与压缩，也不意味着要购买相同数字的磁盘。真实规划需测压缩后的存储、热工作集、更新频率和增长。容量不足时只加内存不能解决长期保留；盲目缩短 TTL 则可能删掉仍需要对账的数据，必须先确认保留合同。

恢复能力同样会限制集群规模。一台落后成员重新同步需要读取、传输和写入数据，其间主节点仍在接收新写。要监控复制历史窗口是否覆盖重建时间，并为备份与重建预留吞吐。只看平时写延迟很低，不代表故障后的全量同步不会影响在线业务。

追问“扫描十万条一定缺索引吗”，先看返回量和选择性。如果本来要返回九万条，集合扫描可能合理；只返回几条却扫描巨大，才更值得检查索引与查询条件。`executionStats` 会实际执行查询，因此生产排查也要限定范围、耗时和权限，不因为叫解释计划就假设没有资源成本。

最后将三分钟回答补成闭环：我先设计文档和验证规则，用单文档原子更新保护局部业务条件；再按确认与读取语义决定故障时能保证什么。跨文档关系用短事务，外部动作仍单独幂等；查询靠计划与工作集验证，复制靠历史窗口和选举验证，备份靠隔离恢复验证，升级靠二进制与特性兼容边界验证。
