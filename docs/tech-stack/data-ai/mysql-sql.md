...
AIOps 不是只有时序指标和模型，很多关键上下文来自结构化数据：告警表、事件表、工单表、发布记录、资产信息、值班记录。SQL 能让你把这些数据关联起来，回答“故障发生前后发生了什么”。
...
- 用聚合统计告警量、故障次数、服务影响。
- 支持报表、复盘和 AIOps 数据分析。
- 为 Python、FastAPI、RAG 项目提供基础数据源。
...
- [ ] 我能写 `CREATE DATABASE` 和 `CREATE TABLE`。
- [ ] 我能用 `SELECT`、`WHERE`、`GROUP BY` 查询告警数据。
- [ ] 我能解释主键、索引、事务和 InnoDB。
- [ ] 我能把告警表和变更表关联查询。
- [ ] 我能说明 SQL 在 AIOps 数据分析中的作用。

## 面试题

1. SQL 在 AIOps 中能解决什么问题？
2. 主键和索引有什么区别？
3. `WHERE` 和 `HAVING` 有什么区别？
4. 事务 ACID 分别是什么意思？
5. 为什么告警表需要按时间字段建索引？
6. 如何查询最近 24 小时告警最多的服务？
7. MySQL 和 Redis 适合存储的数据有什么不同？
8. 慢查询可能由哪些原因引起？
