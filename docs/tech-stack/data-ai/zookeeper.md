...
三个调度器只能一个发出自动化指令，就像三位值班员只能一位拿总钥匙。Session（会话）表示租用身份，TCP 连接表示当前通话线路；电话短暂断了，租用身份不一定立刻失效。只有服务端判定会话过期，相关临时节点才会消失，其他竞争者才按配方继续竞争。

学生：“原主节点卡了十秒又恢复，它还以为自己是主，怎么办？”老师：“这就是协调服务选出新主之后，执行系统仍需要拒绝旧主的原因。”Fencing token（隔离令牌）可以用单调变化的任期或序号，让受保护资源拒绝旧持有者的操作；具体实现必须由下游参与，不能只在客户端检查一次锁存在。

因此分布式锁不是“创建成功一个节点”就完成。还要处理结果未知的重试、会话过期、客户端暂停、网络分区和释放时所有权校验。老师建议生产使用成熟客户端配方，并把故障时间线画清楚；用单节点实验学原语，别把它当成已经证明的跨机房锁。

### Watch 课堂：铃声告诉你变化了，答案要重新读

普通 Watch（监听）被触发后需要重新读取并注册，通知也不是完整变更日志。配置从 A 快速改成 B 再改成 C，客户端可能只需要知道当前是 C；如果业务要求每次变化都可消费，应另用消息或审计日志。持久 Watch 改善订阅生命周期，并不会把 ZooKeeper 变成可靠事件队列。

回调里不要执行长时间网络查询。回调被堵住时，应用可能迟迟处理不了连接状态变化，表面上像服务端没通知。日志里保留收到通知、重新读取完成和配置生效三个时间，才能分清通知链、读取链和应用更新链。

### 故障实验：用版本竞争模拟并发覆盖

前提是上文 `zk-lab` 已启动。在 `docker exec -it zk-lab zkCli.sh` 中执行 `create /lesson-version A`，再运行 `get -s /lesson-version` 读取版本。两个客户端都记住这个版本，例如 0；先在 A 执行 `set -v 0 /lesson-version B`，再在 B 执行 `set -v 0 /lesson-version C`。

预期第一次成功，第二次出现 BadVersion（版本不匹配）。这是正常冲突保护，不是协调服务故障。恢复方法是 B 重新读取数据与版本，按业务决定是否继续修改；不要为了命令成功删除 `-v`。清理前确认路径是本课创建的 `/lesson-version`，用 `delete /lesson-version` 删除这个无子节点教学项，再按原步骤清理容器。若第一次就版本不符，说明路径曾被修改，重读当前版本后重做。

### 多数派与恢复面试

三投票节点失去一台仍有多数，失去两台则不能继续正常多数派写入；Observer（观察者）提供额外读接入，但不补投票人数。机房分布要结合投票权，三节点放两台在同一机房，该机房故障会失去多数。增加节点也增加复制和维护成本，不能只用节点数量衡量可靠性。

事务日志磁盘延迟可能同时拖慢写确认、会话心跳处理和上层选主。AIOps 时间线应连接磁盘延迟、请求积压、会话过期、上层主备切换与业务错误。日志快照清理必须使用受支持机制并保留恢复所需集合，不能看到文件旧就删除。

30 秒解释少量协调状态、会话、监听和多数派。3 分钟以调度器选主说明创建、监听、失联、重新竞争和下游隔离。追问“拿到锁是否永远安全”，回答租约失效和旧持有者问题；事故题设为 GC 暂停导致大量会话过期，先查客户端与服务端暂停证据，再恢复负载和实例，不能无证据无限延长会话超时。
