...
Prometheus / MySQL / Kafka export / CSV（逗号分隔表格文件）
  ...
  -> report（报告） / dashboard（仪表盘） / alert enrichment（补充告警上下文）
...
在按上面方式把训练集传给 `fit`、测试集传给 `predict` 时，Pipeline 会在预测阶段调用已拟合的预处理；如果把全量数据传给 `fit`，它不会自动识别哪些行属于测试集。
...
前提是创建独立的 `sklearn-anomaly-detector` 目录，保存本文八行合成指标，按顺序执行环境安装、训练和检测。不要混入真实业务数据，也不要先运行检测再猜模型文件在哪里。安装后保存 Python、NumPy、pandas 和 scikit-learn 的实际版本；示例采用公开稳定接口，但浮点结果、默认参数和依赖兼容仍由实际环境决定。实验无需显卡、数据库或外部模型接口。

第一轮先运行最短的 `detect_anomaly.py`，预期返回八行，异常标签来自 `1` 与 `-1` 两种值，分数较低的行排得更异常。十点零五分这一行同时有高错误率、高延迟和流量下降，应重点检查它的分数排名；不能由八行样本推导真实事故准确率。`contamination=0.15` 用于选阈值，样本数量很小时离散行数不一定恰好等于百分之十五，也不保证只标出一条。

第二轮运行训练和检测两个独立进程，预期训练保存完整 Pipeline，检测能加载并再次输出八行，生成按分数排序的复核表。检查列名、时间和八个原始样本是否一一对应，防止排序后标签错配。这个实验训练与检测用同一份数据，只证明接口与持久化闭环，不能称为对未来数据的泛化验证。需要评估时，补充独立时间段和事故标签，按前文拆分原则重做。

第三轮执行特征合同故障脚本，预期错列或单位错误被显式拒绝，恢复后可以预测。若第一次脚本运行失败，优先核对虚拟环境、文件当前目录、依赖导入和模型路径；若返回结果不合直觉，先查看原始数据与变换后特征，不立刻调大量超参数。异常检测的“符合老师预期”不是唯一验收，能够解释不符合预期的证据更有价值。

清理时先停止教学进程，确认输出都位于自己新建的实验目录。可以保留模型与复核表作证据；需要重新训练时使用新的运行目录或明确归档旧文件，避免不知不觉覆盖已经引用的结果。移除本课模型和复核表不会删除输入 CSV，但会失去该次训练的精确产物；不能把下一次训练出的相似模型当成原模型恢复。

## 增量学习课堂：多次 fit 不等于逐批学习

学生说：“我把一年数据分成十二份，每份调用一次 fit，是不是就学完一年？”通常不是。许多估计器再次 `fit` 会重新学习，最终状态主要来自最后一次输入，而非自动累计全部历史。`partial_fit` 才是部分估计器用于增量更新的接口，但并非所有模型都提供它，也不是所有 Pipeline 都天然能这样工作。具体支持范围按官方 [大数据计算策略](https://scikit-learn.org/stable/computing/scaling_strategies.html) 核对。

增量更新需要持久化的不只是权重，还有预处理统计、类别集合、已处理数据位置和算法状态。某批数据应用两次，模型可能被重复影响；若先更新模型后记录偏移量，中途崩溃还要解决这一批究竟是否已经应用。对于初学项目，定时从版本化历史数据重新训练通常更容易审计；只有确实需要实时更新，并能管理状态恢复时，才引入在线学习。

`warm_start` 表示某些估计器在后续拟合中复用已有状态，它的用途由具体模型定义，不能作为 `partial_fit` 的同义词。比如树集成增加树数与按新数据持续更新已有树，是不同事情。把接口名称背下来不够，面试时应说明每次调用改变哪些状态、能否撤销、是否依赖输入顺序、失败后从哪里恢复。

这也连接到漂移问题。数据分布变了不一定应该立即训练：可能是采集单位变化、故障样本污染、节假日流量或新服务上线。先诊断变化原因，再决定修输入、分组建模、调整阈值还是重新训练。自动把最近所有数据学习成正常模式，可能把正在持续的事故慢慢吸收成新常态。

## 解释课堂：重要特征不是已经找到根因

如果模型最重视数据库等待时间，只能说明这个特征对当前预测有用，不能直接证明数据库是事故起点。上游流量激增、连接泄漏或下游依赖抖动都可能让它变化。根因分析还需要时间先后、依赖关系、变更证据、对照服务和排除其他假设。AIOps 应把特征解释作为调查线索，不能自动把最高重要性字段写入最终事故结论。

置换重要性通过打乱某一列观察评分下降，可帮助理解模型依赖，但高度相关特征会互相替代。若延迟均值与高分位值表达相似信息，单独打乱某列可能影响不大，不能据此说它们都没有价值。评估还必须在未参与训练的合理样本上做，并使用业务关心的评分；训练集上的解释可能只在解释过拟合。

服务上线后，模型包可信但特征读取无权限，也必须失败而不是补成正常。只读推理账号无需训练数据仓库的删除权限，反馈写入与模型发布应分角色。反馈标签也需要审核：把模型自己的预测当成真值继续训练，会强化已有偏差。保留人工判定来源和时间，区分未复核、确认误报和确认事故三个状态。

## 三分钟完整答案示范

“scikit-learn 用统一估计器接口处理结构化机器学习。我把每个服务窗口整理成一行特征，先写单位、时间与缺失合同，再按部署场景拆分训练、验证和测试。Pipeline 把填补、缩放、编码与模型作为整体拟合和预测，它能维持转换规则，但防不了输入中的未来字段或错误拆分。孤立森林提供相对异常分数，不是事故概率。”

“评估时我同时看窗口精确率召回率、事件发现率与每班通知量，在固定规则基线上验证是否有实际提升。上线保存整个模型包、特征合同、阈值和环境，加载可信产物后运行金样本。接口延迟拆为特征读取、转换、预测与下游发布，内存预算计算每个工作进程的模型副本。遇到分数突变先定位输入、转换、模型还是策略，再选择最小修复。”

“回退必须恢复匹配的输入语义与模型配置，历史样本回放只走影子通道，不重复创建真实工单。若需要增量学习，要额外管理状态与数据位置，不能循环调用 fit 冒充持续学习。这个框架帮助发现模式，最终告警和修复仍需业务证据与授权。”追问“能否直接扩大模型”，回答先用同一评估集证明数据与简单基线已经充分利用，再权衡复杂模型增加的训练、解释和运维成本。
