...
| 行 | 内容 | 说明 |
|---|---|---|
| 第 1 行 | <code>service=payment-api</code> | `service` 是服务名称字段，`payment-api` 是具体服务名，表示这条记录属于这个服务。 |
| 第 2 行 | <code>symptom=接口超时</code> | `symptom` 是故障现象字段，`接口超时` 是故障现象，描述用户或系统看到的问题。 |
| 第 3 行 | <code>database=oracle-prod</code> | `database` 是数据库名称字段，`oracle-prod` 是具体数据库实例或库名。 |
| 第 4 行 | <code>duration=15m</code> | `duration` 是持续时间字段，`15m` 表示持续分钟数，常用于表示故障已经持续多久。 |
...
| 行 | 内容 | 说明 |
|---|---|---|
| 第 1 行 | <code>tablespace</code> | 这一行里的英文要这样读：`tablespace` 是英文标识，通常代表字段名、组件名、文件名、资源名或示例值。 |
| 第 2 行 | <code>  -&gt; datafile01.dbf</code> | 这一行要理解这些英文词：`datafile01.dbf` 是英文术语，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 3 行 | <code>  -&gt; datafile02.dbf</code> | 这一行要理解这些英文词：`datafile02.dbf` 是英文术语，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 第 4 行 | <code>listener</code> | 这一行里的英文要这样读：`listener` 是监听器。 |
| 第 5 行 | <code>  &#124;</code> | ASCII 图里的连接符号，用来辅助表示上下层关系；真正要理解的是它连接的前后组件。 |
| 第 6 行 | <code>  v</code> | ASCII 图里的连接符号，用来辅助表示上下层关系；真正要理解的是它连接的前后组件。 |
| 第 7 行 | <code>Oracle instance</code> | 这一行里的英文要这样读：`Oracle instance` 这个英文标识可以拆开理解为：实例名称字段。 |
| 第 8 行 | <code>  &#124;</code> | ASCII 图里的连接符号，用来辅助表示上下层关系；真正要理解的是它连接的前后组件。 |
| 第 9 行 | <code>  +--&gt; SGA / PGA</code> | 这一行要理解这些英文词：`SGA` 是英文术语，表示本节知识地图里的一个组件、命令、状态或学习主题；`PGA` 是英文术语，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 10 行 | <code>  +--&gt; background processes</code> | 这一行要理解这些英文词：`background processes` 是英文术语，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 11 行 | <code>  +--&gt; redo log</code> | 这一行要理解这些英文词：`redo log` 是英文术语，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 12 行 | <code>  +--&gt; undo tablespace</code> | 这一行要理解这些英文词：`undo tablespace` 是英文术语，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 13 行 | <code>  +--&gt; datafiles</code> | 这一行要理解这些英文词：`datafiles` 是英文术语，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 14 行 | <code>  +--&gt; control files</code> | 这一行要理解这些英文词：`control files` 是英文术语，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 15 行 | <code>  +--&gt; archive logs</code> | 这一行要理解这些英文词：`archive logs` 是logs=日志。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 行 | 内容 | 说明 |
|---|---|---|
| 第 1 行 | <code>primary database</code> | 这一行里的英文要这样读：`primary database` 这个英文标识可以拆开理解为：数据库名称字段。 |
| 第 2 行 | <code>  &#124;</code> | ASCII 图里的连接符号，用来辅助表示上下层关系；真正要理解的是它连接的前后组件。 |
| 第 3 行 | <code>  &#124; redo transport</code> | 这一行里的英文要这样读：`redo transport` 是英文标识，通常代表字段名、组件名、文件名、资源名或示例值。 |
| 第 4 行 | <code>  v</code> | ASCII 图里的连接符号，用来辅助表示上下层关系；真正要理解的是它连接的前后组件。 |
| 第 5 行 | <code>standby database</code> | 这一行里的英文要这样读：`standby database` 这个英文标识可以拆开理解为：数据库名称字段。 |
| 第 7 行 | <code>RAC:</code> | `RAC` 是RAC 是名称、状态或组件标识，真实环境里要结合上下文确认它指的是哪个告警、服务或资源，冒号表示后面要填写或列出这个字段的具体内容。 |
| 第 8 行 | <code>instance A + instance B + shared storage</code> | 这一行里的英文要这样读：`instance A` 这个英文标识可以拆开理解为：实例名称字段；`instance B` 这个英文标识可以拆开理解为：实例名称字段；`shared storage` 是英文标识，通常代表字段名、组件名、文件名、资源名或示例值。 |
...
| 行 | 内容 | 说明 |
|---|---|---|
| 第 1 行 | <code>labs/oracle-aiops-basics/</code> | `labs/oracle-aiops-basics/` 是目录示例，用来告诉读者真实项目里应该把学习证据、配置或代码放在哪里。 |
...
| 第 4 行 | <code>  queries.sql</code> | `queries.sql` 是文件、目录、接口路径或匹配模式示例，用来告诉读者真实项目里应该把学习证据、配置或代码放在哪里。 |
| 第 5 行 | <code>  troubleshooting-notes.md</code> | `troubleshooting-notes.md` 是文件、目录、接口路径或匹配模式示例，用来告诉读者真实项目里应该把学习证据、配置或代码放在哪里。 |
...
| 第 2 行 | <code>  id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,</code> | SQL 语句的续行，通常补充字段、条件、函数参数或子查询结构。 |
| 第 3 行 | <code>  service_name VARCHAR2(100) NOT NULL,</code> | SQL 语句的续行，通常补充字段、条件、函数参数或子查询结构。 |
| 第 4 行 | <code>  severity VARCHAR2(20) NOT NULL,</code> | SQL 语句的续行，通常补充字段、条件、函数参数或子查询结构。 |
| 第 5 行 | <code>  started_at TIMESTAMP NOT NULL,</code> | SQL 语句的续行，通常补充字段、条件、函数参数或子查询结构。 |
| 第 6 行 | <code>  resolved_at TIMESTAMP</code> | SQL 语句的续行，通常补充字段、条件、函数参数或子查询结构。 |
| 第 7 行 | <code>);</code> | SQL 语句的续行，通常补充字段、条件、函数参数或子查询结构。 |
| 第 9 行 | <code>SELECT service_name, COUNT(*) AS alert_count</code> | 选择最终要返回的字段或计算结果，是查询结果表头的来源。 |
| 第 10 行 | <code>FROM alerts</code> | 指定从哪张表读取数据，是 SQL 逻辑执行的起点。 |
| 第 11 行 | <code>WHERE started_at &gt;= SYSTIMESTAMP - INTERVAL '1' DAY</code> | 过滤原始数据行，只保留符合条件的记录。 |
| 第 12 行 | <code>GROUP BY service_name</code> | 按指定字段分组，让每组单独统计或聚合。 |
| 第 13 行 | <code>ORDER BY alert_count DESC;</code> | 对查询结果排序，让最重要或最新的数据排在前面。 |
