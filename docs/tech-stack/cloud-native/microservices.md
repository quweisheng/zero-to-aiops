...
老师先给出一个反常现象：支付服务每次只处理五十毫秒，订单服务却等待两秒后超时。你会先给支付扩容吗？先不要急。订单请求可能还在等连接池、等线程池、等舱壁许可，根本没有到达支付。把总耗时拆成取得执行资格、网络连接、远端处理、读取响应、上游序列化，才能判断钱应该花在哪一层。下游日志没有请求，不证明请求被网络丢弃，也可能仍在上游队列里。

线程池用工作线程处理任务，队列装尚未被线程取走的任务；连接池保存可复用连接或管理并发连接额度。这两个池不是同一件事。把线程上限从二百调到两千，不会让只有二十个数据库连接的服务突然快十倍，反而可能让更多线程抢同一个瓶颈。响应式事件循环也不是无限容量：如果在事件循环中执行阻塞文件读写或长时间计算，少量线程就会被占住。先确认应用是同步请求模型还是响应式模型，再选择线程、连接和阻塞调用的证据。

舱壁隔离的目标是让支付故障不占满订单查询的所有资源。信号量型舱壁用许可限制同时执行数量，不负责额外创建工作线程；线程池型舱壁把工作交给独立线程与有界队列。选后者需要考虑线程切换、上下文传播和排队时间，选前者需要理解调用线程仍在哪里运行。Spring Cloud Circuit Breaker 可能根据引入的依赖启用不同集成，不能仅凭配置名称推断实际实现；应检查依赖树、绑定属性和运行指标。[官方舱壁配置说明](https://docs.spring.io/spring-cloud-circuitbreaker/reference/spring-cloud-circuitbreaker-resilience4j/bulkhead-properties-configuration.html)是核对入口。

超时也不等于远端取消成功。计时器可以让上游停止等结果，但底层网络请求、数据库语句或第三方支付可能仍继续执行。如果超时后立即放开全部额度并再次重试，后台未完成工作与新工作就会叠加。需要在客户端配置连接和读取超时，在业务协议中传播适当截止时间，并验证具体客户端是否真的响应取消。长任务更适合返回受理标识，让调用方查询状态，不要让一个同步连接承担无期限工作。

做一个不用启动服务的容量推演。前提是一张纸：画支付与订单查询两条执行队列，每条最多两个工作位置、一个等待位置。正常轮各放两个请求，预期都开始执行；故障轮让支付的两个请求一直停留，再放入三个支付请求，预期只能一个等待、其余明确拒绝或降级，而订单查询仍能完成。错误方案是把两队列合并：支付占满后查询也无法运行。恢复轮释放支付请求并重新投递一次，验证在途数回到零；清理所有纸面请求卡片。记录拒绝数、最大等待时间和查询可用性，而不是只写“舱壁能隔离”。这个模型没有证明任何框架默认线程数，却能检查设计有没有真正独立的资源边界。

### 八、优雅停机不是等固定秒数再强杀

滚动发布时，一部分实例要离开服务。正确目标是停止接收新工作，让已经接收的工作在有限时间内收尾，并让调用方及时更新可用实例。这里至少有三种时钟：入口和发现信息传播、应用完成在途请求、容器运行时最终终止进程。它们属于不同层，不会因为都填写三十秒就自动配合。特别是已有长连接仍可能向旧实例发请求，要在目标服务器实现和真实入口上验证。

Spring Boot 支持优雅停机，`spring.lifecycle.timeout-per-shutdown-phase` 控制停止阶段的等待预算，而不是任何业务事务都必然完成的承诺。应用必须收到能触发生命周期关闭的信号，直接强杀或某些开发工具的停止方式可能绕过流程。[官方优雅停机说明](https://docs.spring.io/spring-boot/reference/web/graceful-shutdown.html)也指出，不同服务器拒绝新请求的方式可能不同。不要从本机一次 `Ctrl+C` 成功推断 Kubernetes 发布期间绝不丢请求。

生产变更先问：最长允许请求多长？停机窗口能容纳它吗？消息消费者是否先停止取新消息、再确认已完成消息？若未完成工作被重新投递，幂等记录能否挡住重复副作用？后台定时任务和 HTTP 请求不是同一生命周期，要分别记录负责人、停止条件和恢复策略。超出预算时应让未完成工作以可查询、可重试的状态留下证据，不能简单把它记成成功。

事故演练可以先画时间线：旧实例在第零秒被摘流，第两秒入口更新完成，第八秒最后一个已接收请求完成，第十秒进程退出。正常预期是业务连续且没有遗留任务；故障轮把强制终止改到第五秒，标记尚未完成请求的未知结果；恢复轮增加受控等待或缩短业务最大耗时，并验证最终对账。清理只恢复时间线标记，不触碰运行中的进程。真实测试时再使用独立服务、模拟耗时和观测日志检验这些假设，本轮没有执行真实停机实验。

### 九、管理端点的暴露、授权和采集是三道门

配置 `exposure.include` 表达哪些端点通过相应协议可见，不等于所有访问者都获得授权，也不等于监控系统能跨网络访问。反过来，配置了认证也不能弥补把敏感堆转储、环境配置、可修改日志级别的端点无差别暴露到公网。先把端点按健康摘要、只读指标、敏感诊断、运行时写操作分类，再决定管理网络、身份、权限、审计与限额。[Actuator 端点指南](https://docs.spring.io/spring-boot/reference/actuator/endpoints.html)应按现场 Boot 分支阅读，不能将新版默认值直接套进旧应用。

排查时先判别连接失败、路径不存在、权限拒绝还是结果正常但采集缺失。它们分别指向网络与监听、暴露配置、认证授权、抓取配置与数据后端。日志中只记录状态、端点与脱敏身份，不记录令牌。面试追问“为什么不把所有诊断端点开放给 AIOps”时，应回答模型只需要完成任务的最少证据；扩大读取范围会增加泄密风险，扩大写入权限会增加误操作影响面，观测便利不能替代授权边界。

最后给自己一道独立作业：选择一个日常维护的 Java 服务，只画读取业务数据的一条请求路径，标出等待位置、超时责任人、结果状态和退出条件。若某一格只能写“框架会处理”，就回到运行配置与日志补证据；这比一次引入全部组件更能暴露真正的知识缺口。
