...
尖括号只是待替换说明，不可以原样执行：将整个 `<目标节点的-cilium-pod>` 换成刚核对的 agent Pod 名并保留引号，防止 Shell 把尖括号解释为文件重定向。`endpoint list` 查看本节点端点及身份，`policy get` 查看节点接收的策略状态，Hubble 观察近期流。记录源/目标、协议、时间与 verdict（处理结论），不只截一行含 `DROPPED` 的历史事件。先发起一次带三秒超时的新请求，再在同一时间窗观察；旧事件不能说明刚修改后的结果。
...
## 主机安全课堂：审计通过与真正拦截之间还差什么

前面的身份实验针对普通 Pod，现在考虑节点上的 SSH、kubelet 和使用主机网络的 Pod。Host Firewall 是主机防火墙，保护对象与普通工作负载策略不同；`nodeSelector` 按节点标签选择主机，不是把 `endpointSelector` 换个拼写。设计前先画流量在哪个网络命名空间终止，再决定使用何种策略。不能根据容器有一个名字，就判断它一定走普通 Pod 的隔离路径。

官方[主机防火墙文档](https://docs.cilium.io/en/stable/security/host-firewall/)提醒先审计所需管理通信，避免把 agent 到 API server 的链路一并截断；审计模式下本应被拒绝的流量会留下审计证据而不被该策略实际丢弃。它适合限定范围的观察，不是已经生效的安全防线。特别注意本文版本系列中端点审计设置不会跨 agent 重启保留，不能把临时审计状态当成可持久恢复的配置。

请预测：策略忘了放行 API server，当前审计模式下看起来正常，随后节点维护重启了 agent，可能怎样？关键不是“节点重启会清空所有规则”，而是不能继续假设临时审计豁免仍存在。一旦控制链路被拦，控制面上删除策略也可能暂时传不到该节点。恢复通道必须独立于被保护的链路，包括经过授权的控制台或带外入口，具体紧急恢复按目标版本运行手册执行。

本课只做纸面生产模拟，不给共享节点下发主机规则。准备三行通信：管理员到节点、节点到 API server、监控到节点，各自填来源、目的端口、节点标签和预期允许。基础步骤是在审计报告中逐行找到允许证据；故障步骤删除“节点到 API server”这一行的允许条件，模拟从审计切到强制，预期失去配置同步能力。恢复步骤先标记停止继续发布，再说明如何经独立入口恢复同步，最后补回规则并重新验证三行。清理仅丢弃模拟表，未对真实端点开关审计。

排查闭环要包含反例：没有观察到管理流量，可能只是观察窗口内没人登录，并不说明它不需要权限。至少覆盖日常管理、证书轮换、节点加入、监控抓取和故障恢复所需链路。不能让自动学习系统把安静时段的流量集合直接生成永久白名单，低频但关键的控制请求最容易被误删。

## 升级深讲：镜像已就绪，为什么旧连接仍可能断

升级有三种不同工作：预先拉取目标镜像，更新控制组件，改变运行中的网络处理状态。预拉取降低现场镜像下载失败与等待风险，但不证明策略兼容，也不保证连接永远不断。将三者分开验收，才能回答“升级前检查都绿了，为什么应用还有错误”。参考[官方升级指南](https://docs.cilium.io/en/stable/operations/upgrade/)，本文保留前述固定实验版本，不把随时间变化的稳定版页面直接替换成安装命令。

对直接经过内核的三四层流量和经用户态代理处理的七层流量，应准备不同的中断预期。代理重建时，连接端点本身可能变化，应用必须具备有界重连与正确重试。重试读请求与重试扣款请求不是一回事：前者通常容易恢复，后者需要幂等标识和服务端去重。网络升级检查不能替应用承诺“没有重复业务操作”。

变更前保存的是经过审查的期望配置，不只是随手复制当前输出。旧值文件中可能含已经弃用字段，新 chart 可能改变默认项；跨次要版本不应盲目使用 `--reuse-values`。逐项核对路由、地址分配、API 入口、代理模式、观测与安全配置，说明每项为何保留或替换。维护窗口内不要同时切换路由模式、开启新功能和升级内核，否则失败后难以建立因果链。

回滚也要先判断是否已使用新版本特有资源。恢复旧镜像不意味着旧程序能理解新资源，更不意味着外部路由配置已回退。课堂变更单分为“尚未开始”“部分节点更新”“新功能已启用”三格，分别写暂停条件、允许回退范围和需要额外恢复的状态。第一格最容易取消，第三格通常需要更谨慎的兼容检查，不能共用一条回滚命令。

生产设计题可以这样作答：先在相同内核与功能组合的隔离集群演练，预取镜像并检查策略，再小批更新节点；持续验证新连接、长连接、代理请求和控制同步，达到错误预算停止线即暂停。恢复后对比更新前基线，不只看 agent Ready。Kubernetes 节点可调度与网络具备全部业务能力，并非等价命题。

## 观测容量课堂：流事件不是无损审计账本

Hubble 的流事件适合定位路径和处理结果，但完整业务审计应有独立的持久化和完整性设计。流从节点产生，经过缓冲、聚合、过滤和消费者处理，每个环节都有容量与延迟。单次 `observe` 的输出只是当前查询范围内可取得的观察，不是全网每个包的永久副本。对事件丢失的检测也应成为监控自身的监控。

做一个容量估算：假设合成环境每秒产生两万条事件，消费者稳定处理一万五千条，即使业务请求没有失败，观测积压也每秒增长五千条。缓冲只能延后耗尽，不能改变长期生产速度高于消费速度这一事实。应限制不需要的事件、扩展受支持的消费能力、设计保留期限，并保留观测覆盖率；不能通过隐藏丢失指标把仪表盘变绿。这里的数字用于算术练习，不代表 Cilium 产品上限。

基础纸面步骤是按一秒为单位记录生成量、消费量和积压，连续十秒应得到五万条新增积压。故障步骤把消费者暂停两秒，再恢复原处理速率，预期积压不仅不会自动归零，还继续以差额增长。恢复步骤把生成量降到一万，消费者保持一万五千，计算消化积压所需时间。清理时移除本轮计算表，不修改真实 Hubble 参数。若算出暂停结束后立即清零，说明把“恢复消费”误当成“补齐历史”。

AIOps 在这种情况下应该把结论标为“观测覆盖不足”，而不是“没有发现丢包，所以网络健康”。关联业务错误率与事件丢失时间窗，区分业务故障和取证链故障。日志或流量中的地址、域名、用户标识也可能敏感；仅保留必要字段，控制查询权限和保存期限，避免为了训练模型把生产原始流导出到公开仓库。

### 独立口述作业：让同一个故障有可证伪的假设

给你三个现象：新请求超时、端点已就绪、目标节点没有 Hubble 事件。第一种假设是源节点策略拒绝，预测源端存在拒绝证据；第二种是转发路径失败，预测源端允许而目标端收不到；第三种是观测链缺失，预测独立的业务日志或抓包仍有到达。分别写下一条最小检查和能推翻假设的结果，不先选定工具再解释所有输出。

最后把故障实验中的允许、拒绝、修复后允许与永久禁止四种结果归档，附节点落点和时间。若只有一张成功截图，不能证明身份授权正确。本文新增主机防火墙、升级与观测积压案例均为设计或纸面验证，没有启动 Cilium、切换主机防火墙或执行网络升级；产品运行行为仍须由读者在独立实验环境复现。

最后检查实验身份：安装前确认当前上下文属于本轮新建的专用集群，同名集群已存在时不要复用。清理命名空间之前列出里面的工作负载与策略，只在确认全部为本课创建后删除；共享集群不能执行删除 kind 集群的收尾步骤。诊断包也属于实验产物，检查是否包含节点地址、令牌或证书路径后再分享。静态阅读通过、纸面算术正确、隔离环境实验通过、生产变更通过是四级不同证据，学习记录必须标明达到哪一级，不能把本篇命令齐全写成已经完成生产验证。
