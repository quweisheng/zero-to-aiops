...
先在新的专用学习目录操作，确认没有同名版本记录或渲染文件，避免覆盖自己的既有证据。下面索引查询只取得一个候选 Chart，不表示它已经与本文产品版本配对；继续渲染前核对显示的 `appVersion` 与兼容说明，必要时把变量改成已批准的精确 Chart 版本。结束时保留脱敏结果，或只移除这两份本轮生成的文件。仓库索引是客户端配置变化，不会创建 Harbor 实例。
...
老师给你一个需求：“外网慢，能否让所有节点先从 Harbor 拉公共镜像？”这里可以评估 Proxy Cache，即代理缓存项目。普通项目由有权限的人主动推入制品；代理缓存项目连接上游仓库，用户拉取时按需取得并保存内容，不能把它当普通项目继续推送。先搞清楚谁是内容来源，才能设计同步、授权和故障策略。

按本文分支的[代理缓存说明](https://goharbor.io/docs/2.15.0/administration/configure-proxy-cache/)，未命中的镜像需要从上游取得；再次访问时可能检查上游清单，内容更新则取得新内容。上游不可达与上游明确报告对象已不存在是两种情况，不能一律推断返回旧缓存。所以这项能力适合减少重复下载和改善访问，但不能代替受控制品晋级与长期保留。关键发布依赖应复制或推入受控项目，并验证摘要、授权和恢复条件。

权限还有一层容易漏：Harbor 使用配置给上游连接的账号访问内容。给缓存项目的读者开放访问时，要评估这个上游账号能看到哪些私有仓库，否则上游账号的宽权限可能通过缓存项目扩大给更多人。不要为了首次连接方便使用个人全权限账号。项目成员、上游权限、令牌服务地址与出口网络应一起审查；令牌服务也可能不在仓库同一个域名，出口白名单要基于可信服务链核对，而不是允许任意重定向。

如何排障？正常缓存命中应关注响应时间、出网请求与后端读取；冷缓存失败应关注上游认证、限额、DNS、证书和容量；同一标签内容变化则先检查上游摘要变化，不急着判定本地数据损坏。只测已经缓存的小镜像会漏掉首次下载和大镜像路径。验收至少分为命中、未命中、上游暂不可达三个场景，并明确哪些属于官方支持行为、哪些需要现场验证。

做一个纸面基础实验：在表里列镜像甲已缓存、镜像乙未缓存，以及两者的固定摘要。正常轮把上游设为可访问，预测两者可完成拉取，但乙需要上游数据；故障轮只把上游改成不可达，甲可能按支持行为使用缓存，乙不能凭空出现；恢复轮重新开放上游，确认乙取得正确摘要后进入缓存。验证时分别记录“内容是否已有”“上游结果”“用户结果”，而不是只写仓库健康。清理只恢复表中状态，没有改网络、缓存规则或真实制品。本轮未执行产品代理缓存故障测试。

## 保留规则课堂：先算集合，再谈删除

保留规则表达“留下什么”，不是给删除命令加一个筛选条件。按[官方规则说明](https://goharbor.io/docs/2.15.0/working-with-projects/working-with-images/create-tag-retention-rules/)，规则间采用或关系，多个命中的保留集合合在一起；先匹配仓库，再按数量或时间与标签条件判断。范围与规则顺序的语义不能靠界面中文猜。特别是“每个仓库最近几个”不是整个项目共同分享一个总名额。

请算一遍：虚构项目有订单、支付两个仓库，各三份独立制品。第一条保留每仓库最新两份，第二条保留用于回退的标签。若订单最旧的一份恰有回退标签，它应额外留下，因此最后不一定正好四份。再考虑一个制品同时有普通标签与受保留标签：按该版本文档，保留以制品为单位执行时可能把该制品及其多个标签一起留下。因此看到标签数没有按想象下降，先查共享制品关系，不要认定任务失效后直接删文件。

这段练习升级为故障轮：故意漏掉回退规则，圈出下一次执行会使恢复依赖丢失的摘要。预期风险来自发布清单与保留集合不相交，而不是磁盘立即报错。恢复轮补回经批准的规则，在预演结果中逐一核对当前生产、可回退版本和相关签名。清理删除纸面候选清单即可；若在真实测试项目练习，必须先保留可重新取得的测试副本，不能用生产制品做删除验证。

无标签制品也要显式纳入评审。按摘要部署可以不依赖标签，但回收策略未必知道哪些摘要正在生产使用。应从部署与资产清单反向得到保护集合，再对照仓库规则。不能让“最近没有拉取”自动等于“没人用”：一个稳定运行半年没有重启的服务，可能正依赖那份半年未被拉取的镜像。恢复时才发现仓库已清掉它，是典型的容量优化损坏恢复能力。

## 运维状态课堂：任务完成、策略通过和可发布分别看什么

扫描任务有排队、执行、成功、失败等过程，成功表示执行完成；报告里的漏洞结果才描述本次扫描发现；发布策略再决定是否接受该结果。三个状态不要压成一个布尔值。复制亦如此：任务完成要结合目标摘要核验，保留完成要结合预演与保留对象，垃圾回收完成要结合实际回收量和存储指标。一个总览绿色灯无法代替这些不同的证据。

AIOps 可按同一制品摘要维护一张状态卡：构建来源、推送时间、扫描时间、漏洞库时间、签名身份、批准状态、目标站点可用性和当前部署引用。状态卡更新要注明观察时间；采集断流时保留最后已知值并标注陈旧，不把它当当前确认。若模型建议“删除旧制品”，必须给出哪些发布依赖被排除的证据，执行仍经过审批与预演。

容量指标也要分口径。项目配额是治理边界，后端真实空闲量是物理承载，上传临时内容与共享层又影响两者差异。某项目未到配额不证明磁盘仍有安全余量；存储仍有余量也不意味着用户有权继续推送。遇到拒绝先读具体错误，再对照项目配额、制品大小、存储容量与任务状态。随意提高所有配额可能只把局部限制改成整个仓库容量事故。

最后做一题交付设计：断网园区需要保留两个月的可回退版本，应优先设计受控制品导入、完整性核验、独立保留与备份，而不是仅部署代理缓存。离线物料包括清单引用的全部层、对应平台内容、所需关联证明、扫描数据库与许可材料；厂商镜像是否允许再分发需另行核对。恢复验证应从一台无旧缓存的隔离客户端开始，按关键摘要逐一读取，记录未覆盖的架构与版本。这才是把软件仓库变成可恢复交付依赖，而不只是搭起一个网页。
