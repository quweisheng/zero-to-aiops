...
用户点击 router-link / 调用 push
  ...
  -> 确认导航
  -> 更新 currentRoute
  -> afterEach（导航后置钩子，可记录导航结果）
  -> 触发 DOM 更新并渲染 RouterView（路由视图）
```

后置钩子不等于新页面全部绘制完成，更不保证页面接口已成功返回；实际流程与异步守卫细节见 [Vue Router 完整导航流程](https://router.vuejs.org/guide/advanced/navigation-guards.html#The-Full-Navigation-Resolution-Flow)。
...
## 版本边界课堂：同样写解构，为什么结果不同

普通 `reactive` 对象的数值解构与 Vue 3.5 的响应式 props 解构，外观相似却不是同一机制。后者依赖单文件组件编译器：在同一 `script setup` 中，编译器识别从 `defineProps` 解构出来的变量，并把相关读取改写为 props 访问。不是 JavaScript 的解构规则突然改变了，也不意味着任意对象、任意文件里的解构都会持续响应。[响应式 props 解构](https://vuejs.org/api/sfc-script-setup.html#reactive-props-destructure)

把这个解构值直接传给普通函数，仍可能只是把当时的值传过去。需要让组合式函数之后重新读取时，应按接口约定传 getter（读取函数）或 ref，让消费者知道怎样取得当前值。比如监听某个 props 条件，传一个读取它的函数表达依赖，与直接传入一次读取的字符串不同。排障先标出编译转换范围，再检查函数参数契约，不能把所有“解构失效”都套同一个修复。

因此升级 Vue 时要同时对齐运行时、单文件组件编译器和类型工具。开发机新编译器生成了响应式转换，CI 却用旧依赖，可能出现行为差异。锁文件、实际解析版本与编译产物是证据；仅查看 `package.json` 的版本范围不够。教学固定系列是为了让同学可重复观察，不代表可以忽略编译工具的配套关系。

### 浅响应：省下追踪成本，也要明确修改协议

给图表库实例加响应代理也不一定有意义。实例可能包含浏览器句柄、循环引用和自己的状态系统，先核对库的集成说明；只把需要展示的业务摘要交给响应层，实例句柄由组件生命周期管理，能让依赖图和清理边界更清楚。

`shallowRef` 只在外层 `.value` 访问处建立响应关系，内部对象不会因为这个容器自动深度转换。它适合大型不可变数据快照、外部状态或某些第三方实例，但内部字段原地变化不保证触发依赖更新。若选择浅响应，团队就要约定替换整份快照，或在明确场景调用触发接口；不能一半代码按深响应写，一半代码按快照替换写。[浅响应容器](https://vuejs.org/api/reactivity-advanced.html#shallowref)

课堂可以在现有个人 Vue 实验中把状态写为 `shallowRef({ count: 0 })`，模板读取其中计数。先点击只执行内部 `count++` 的按钮，预期该修改本身不触发依赖更新；再点击把 `.value` 替换成新对象的按钮，预期更新。注意其他响应式变更若偶然促成重新渲染，也可能把内部新值显示出来，因此不能靠“刚才看见变了”证明它是深响应。用专用组件、单一按钮和开发追踪工具隔离变量，才有清晰因果。

前置条件仍是已创建且可运行的个人练习副本；操作前保留原组件。验证时记录按钮代码、依赖触发与 DOM，而不是只截图最终数字。修复为一致的快照替换后恢复原组件，停止个人开发服务即可清理。这个实验不连接真实告警，不以一次渲染速度说明生产性能收益；真正收益还需在相同数据规模下测依赖数量、内存和更新时间。

面试三分钟串讲可以从“读到的是普通值还是响应容器”开始，再讲编译器宏的专门转换、跨函数边界传 getter、浅响应的更新协议，最后用版本不一致或误原地修改解释事故。AIOps 实时看板若显示旧快照，先确认数据版本与更新路径，再考虑性能优化；错误但流畅的画面不能作为健康证据。
