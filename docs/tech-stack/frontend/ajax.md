...
| 403 | 服务器理解请求但拒绝执行 | 清楚显示权限边界 | 不能据此保证已经认证，与 401 语义不同 |
...
需要 Node.js 18+（内置 Fetch 不是本实验重点，主要用其 HTTP server），一个只属于本课的空目录，以及未占用的本地 4177 端口。先创建下述两个文件再启动，不使用生产账号或接口。创建 `server.mjs`：
...
## 进阶老师追问：看得见响应，为什么取不到追踪号

浏览器网络面板展示响应头，并不代表页面脚本有权读取所有响应头。跨源响应会经过过滤，服务端若希望脚本取得自定义追踪号，需要用 `Access-Control-Expose-Headers` 明确暴露相应字段。它与预检中的允许请求头不是一个方向：前者决定脚本可读哪些响应元数据，后者决定请求可带哪些非安全列表字段。把两者混写，往往表现为网络面板有追踪号而 `headers.get()` 返回空。

老师让你先判断缺失发生在哪层：服务端没生成、代理删掉、浏览器不暴露，还是字段名与契约不一致。使用合成追踪号逐层核对即可，不需要开放所有头，更不能为了诊断暴露 Cookie 或授权信息。携带凭据时，通配暴露头还存在特别语义，应采用明确字段清单。前端拿不到追踪号时保留本地操作标识和错误类别，不能伪造一个后端链路已经接受的标识。[响应头暴露规则](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Access-Control-Expose-Headers)

### 刷新凭证也需要并发与身份世代

想象同一页面同时发出五个读取请求，旧凭证失效后全部收到认证失败。如果每个请求各自刷新，可能出现多次轮换互相覆盖，甚至把刚获得的新会话覆盖成旧结果。一个常见设计是把同一身份下正在进行的刷新操作共享起来，让后续请求等待同一结果；但是等待必须有时间上限，失败后结束队列，不能无限递归地再次刷新。

共享刷新不是跨身份共享。用户在刷新期间退出或切换租户，旧刷新结果不得重新激活先前身份。可以把身份变化视为新的会话世代：开始请求时记录世代，提交结果前核对它仍然有效。这个检查同时约束响应数据、错误提示和凭证更新。清除一个存储键却让旧 Promise 稍后写回，并没有真正完成退出。服务端吊销、会话轮换和权限校验仍是最终保障。

这里也有重试边界：认证成功后能否重放原请求，要看原操作是否可能已经产生业务效果，以及请求体是否还能安全重建。流式请求体不能随意重复消费；敏感写请求即使拿到新凭证，也应遵循原来的幂等键和操作查询约定。不要把请求拦截器写成“任何错误都刷新并重发”的万能补丁。

### 服务端让你稍后重试，究竟等多久

`Retry-After` 可以表示秒数，也可以表示一个 HTTP 日期。只用数字转换读取它，会在日期形式时得到无效值；把秒数直接当毫秒，又会把应当等待的时间缩小千倍。解析后还需要与客户端剩余总预算比较，并约束不合理的负值或过大值。绝不能因一个异常响应头让页面永远转圈，也不应因为等待超出预算就提前违背节流要求强发请求。[等待头的两种形式](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Retry-After)

如果服务端建议一分钟后重试，而用户本次操作只允许再等五秒，合理结果通常是结束自动等待并解释何时可再次尝试。若使用日期形式，要意识到客户端时钟可能不准。这里先记录原始头、解析结果和决定，不在日志里记录完整业务响应。重试属于有预算的状态机，不是一个隐藏在工具函数里的无限循环。

### 克隆响应：两个人读取不代表两份成本消失

`Response.clone()` 让两个消费者各自读同一来源的正文，但读取速度不一致时会产生缓冲。假如界面快速消费一个巨大下载，审计分支却一直等着把整个正文转成字符串，慢消费者的缓冲可能不断增长。它不是复制一个轻量指针就自动得到无成本多播。监控日志应优先记录状态、大小和经过脱敏的必要摘要，不为方便排障克隆每个完整响应。[克隆响应与背压](https://developer.mozilla.org/en-US/docs/Web/API/Response/clone)

课堂自测不需要访问接口：列出成功、认证过期、超时未知、用户退出、会话切换五个事件，再写出每个事件之后哪些待处理结果仍有资格修改界面。预期是退出后旧结果全部失去资格，超时写操作进入待确认，刷新失败不会留下永久等待者。用纸面事件序列验证后，删去模拟笔记即可；它证明的是状态设计思路，不冒充浏览器真实网络测试。面试三分钟回答时，把头过滤、刷新并发和身份世代连到这条事件序列，能解释为什么一个请求封装函数不足以替代完整的客户端状态模型。

## GitHub 学习证据清单

再做一次反例检查：如果刷新后五个请求一起重放，接口仍可能出现瞬时尖峰。共享刷新只减少认证请求数量，没有自动限制业务并发，因此重放队列还要受并发上限和剩余预算控制。若其中一个查询已经被新筛选替代，应直接撤销其重放资格；否则虽然结果最后被丢弃，服务器资源仍然被浪费。排查时分别统计刷新次数、排队数、实际重放数和被替代数，不把“总请求减少”直接当成所有竞态已解决。将这组计数放入合成测试记录，既能证明优化作用，也能发现只是把请求推迟集中发送的假改善。
