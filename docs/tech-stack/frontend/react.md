...
本文以 React 19.2 系列为教学基线，在线文档会继续演进，不能据此自动升级项目。本仓库自身锁定 React 19.2.x 作为客户端基线。React Server Components（RSC）对应用开发者已有稳定能力，但框架/打包器依赖的底层实现接口可能不遵循 minor 级语义兼容；不要脱离受支持框架自建后默认可随意升级。
...
React 19.2 的 `useEffectEvent` 可把 Effect 中需要读取最新值、但不应触发重新同步的逻辑分离出来。它不是绕过依赖的通用工具，不能在普通事件或任意位置调用；按对应官方规则和 lint 版本使用。
...
## 订阅收尾课：快照不能每读一次就凭空变一次

外部事件仓库可能已经被别的框架或原生代码维护，React 需要知道何时变化、怎样读取同一版本。`useSyncExternalStore` 将这个契约拆成订阅函数、快照读取函数以及服务端初始快照函数。订阅函数返回解绑方法；读取函数应在数据未变时返回相同快照，通常通过缓存不可变结果实现，而不是每次创建一个内容相同的新对象。[外部仓库订阅契约](https://react.dev/reference/react/useSyncExternalStore)

老师会给出两个错误方向。每次读取都生成新对象，React 可能不断认为快照变化；永远返回同一个可变对象并就地改字段，又可能使变化无法被身份比较识别。正确设计是业务数据确实变化时生成新快照，其他读取复用该版本。订阅通知与快照版本应一起推进，组件卸载后应停止通知该消费者。

可以只用纸面合成仓库练习：初始版本零包含两条告警，连续读取三次应得到同一身份；新增一条告警后版本一产生新身份；再次读取仍复用版本一。故障注入是把每次读取都标为新版本，观察它破坏了什么判定；修复后恢复“修改才换版本”。这个练习没有运行 React 调度器，只验证仓库契约，笔记可直接删除或作为设计证据保留。

服务端渲染还要求服务端初始快照与客户端水合初始快照一致。不能在服务器用租户甲的数据渲染，再让客户端第一次读取空仓库或租户乙。传递数据要使用框架认可的安全序列化方式，并隔离每次服务端请求，不能把一个全局可变单例当成所有用户的仓库。快照一致性解决显示问题，可信身份与授权解决访问问题，两者缺一不可。

面试追问“外部仓库为什么不用普通 Effect 随便订阅”，先说明渲染和订阅之间的变化窗口、快照一致性及清理需求，再说明采用官方适配接口能让 React 协调读取。性能追问则检查通知频率、快照粒度和数据规模；不能为了减少渲染而隐藏真实业务变化。告警洪峰时可以按明确业务规则合并展示更新，但审计记录仍需保留应有的事件事实。
