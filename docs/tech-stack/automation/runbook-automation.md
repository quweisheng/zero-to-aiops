...
上面的 HighErrorRate 是高错误率告警；service 指服务，environment 指环境，error_rate 是错误比例，started_at 是事件开始时间。production 表示生产环境，时间结尾的 Z 表示协调世界时，不能直接当成本地时间。
...
这份英文模板是结构示范：Metadata 记录负责人、版本、复核时间、风险和适用环境；Trigger 与 Inputs 定义触发条件和参数；Preconditions 检查服务标签、环境允许值及响应者身份。Context To Collect 要求最近发布、错误率、延迟、主要日志、重启次数与下游状态；Decision Rules 只是形成回滚或隔离的候选方向，不授予执行权。Steps 先生成摘要，再附证据，任何生产变更先审批；Verification 检查错误、延迟和严重告警；Rollback 与 Forbidden Actions 规定停止、恢复及禁止操作。英文结构中“只有一台异常就隔离”仍需补足剩余容量和状态性检查，不能脱离后文安全前提直接执行。
...
这段伪代码把 recent_deployment 与错误出现顺序作为发布相关候选，把 only_one_instance_bad 作为单实例异常候选，把 dependency_error_in_logs 作为下游故障线索；recommend 只表示建议，escalate to human 表示交给人工，不是直接调用回滚或重启命令。不要把所有判断都丢给 LLM。关键生产动作必须有确定规则和审批。
...
def main() -> int:
    ...
    if not isinstance(environment, str) or environment not in {"staging", "production"}:
...
作用：查看 runbook workflow 日志。把整个 `<run-id>` 替换为本次获批运行的编号，保留单引号；裸尖括号会被 Shell 当作输入输出重定向。
...
```text
lock_key = conflict_resource_id + environment
```
...
假设采集间隔三十秒，查询使用过去五分钟窗口。你刚增加一个工作实例，十秒后再次查询，读到的仍主要是扩容前的请求。若系统看到错误率没降就继续扩容，短时间内可能创建很多实例，却没有给新实例预热和观测留时间。这不是动作接口失效，而是执行时间、数据时间和判断时间没有对齐。

每种动作应声明预计生效时间、最短观察窗口和最大等待时间。扩容后先确认实例就绪，再看它实际收到请求，最后评价适用指标；三个步骤不可直接压缩成一次查询。观察窗口内不再启动同类修复，超时则进入未知或失败分析，而不是无限续期。业务变化本身也会影响指标，所以记录同期流量和下游状态，不能把自然恢复全部算成功劳。

迟滞是进入和退出使用不同条件，减少阈值附近来回切换。例如合成策略在持续高负载后允许扩容，在较低负载维持更久后才允许缩容，中间区间保持现状。这里没有通用百分比：阈值需要结合容量、业务季节性和错误预算设计。冷却期则限制动作频率；它不能替代授权，也不能证明冷却结束后一定安全，仍需重新采集证据。

### 错误阈值控制新派发，不撤销已经开始的动作

批量操作常有最大并发和最大错误数。并发决定同时暴露多少目标，错误阈值决定何时停止继续发送。两者必须一起推理：即使刚达到错误阈值时立刻停止新派发，已经运行的目标仍可能继续失败，因此最终失败数可以超过阈值。AWS Automation 的规模控制文档明确区分这两个限制，不能把错误阈值当作严格的最终失败上限。[规模执行与速率控制](https://docs.aws.amazon.com/systems-manager/latest/userguide/running-automations-scale-controls.html)

老师用自建调度器的简化策略举例：十台虚构机器，最大并发三，规则明确为“第一例失败就停止新派发”。假设甲乙丙已经同时运行，甲先失败触发停止继续派发，乙丙已经在执行。你不能宣称“最多坏一台”；还应评估这三台是否位于同一故障域、是否超过可用容量。这里不是 AWS 的实际派发顺序或参数映射；产品对绝对错误数、百分比和逐步放量的定义应分别查文档。高风险步骤先小批验证，再按已确认健康结果扩批，不能只靠降低单个参数获得虚假的安全感。

平台停止按钮还要区分停止派发、请求取消与远端动作确实结束。动作收到取消请求后可能仍在提交事务，也可能根本不支持中断。值班台应展示实际运行中的目标与未知状态，不把“已发送取消”写成“全部停止”。某些无法安全中断的步骤，应让它在受控范围完成，再停止后续步骤；行为必须在手册里预先说明。

### 两种修复互相竞争时，先确定共同目标

一个手册因错误率高回滚，另一个因处理量高扩容，第三个因成本高缩容；它们分别看起来合理，却可能在同一服务上相互抵消。按手册名称加锁无法解决资源冲突。应以受影响服务、集群或部署对象识别共同写入边界，由统一策略确定当前事故阶段允许哪些动作。

例如事故恢复阶段暂缓成本优化，先保护最低可用容量；回滚进行时暂停同目标的版本升级，保留只读诊断。这个优先级来自团队政策和事故负责人，不应由模型临时猜测。审计记录被抑制的动作及原因，可以解释为什么某个推荐没有执行，也避免下次重复提交相同建议。

### 离线时间线实验：等证据，不是连续重启

前置条件是读懂本地推荐器的正常与异常输入，本练习只在笔记使用虚构时间。写下零秒执行扩容，二十秒实例创建，六十秒就绪，九十秒收到请求，一百二十秒取得第一批新实例指标。故障注入是在十秒看到旧窗口仍有错误，提出再扩容。预期策略应拒绝重复动作并说明仍在观察，而不是把旧指标解释成此次扩容失败。

再把九十秒的“收到请求”删除，预期后续验证先查路由，而非把空指标当成零错误率。验证方式是每个判断写明依据的数据时间和覆盖目标；如果结论用了动作发生前的数据，就标记证据不适用。恢复该记录后重算流程，确认只在新的有效样本足够时评价恢复。清理删除合成时间线即可，无服务器、队列或云资源需要停止。

面试追问“自动化执行次数越来越多，恢复反而慢”，三十秒先怀疑反馈延迟、重复事件和冲突动作，不先提高并发。三分钟给出事件去重、资源互斥、动作生效时间、观测窗口、停止扩批和人工接管证据。设计题则把业务恢复作为目标，分别统计推荐正确、动作成功和业务恢复，防止用大量执行次数掩盖一个持续未解决的事故。
