...
| 部分 | 含义 |
|---|---|
| `resource` | 块类型，表示声明受管资源 |
| `"local_file"` | 资源类型，由 local 插件提供 |
| `"hello"` | 当前模块内的资源名称 |
| `{ ... }` | 块体，包含这个对象的参数 |
| `filename`、`content` | 参数，分别设置路径与文件内容 |
...
后续命令中的整个 `<address>` 要替换为核对过的 Terraform 资源地址，`<id>` 替换为提供方要求的真实资源标识，保留单引号；它们不是可原样执行的参数，地址也不等于资源在云控制台上的显示名称。
...
把整个 `<LOCK_ID>` 换成已核对来源和归属的锁编号，保留单引号。但这是危险操作，不要在不知道谁持锁时强制解锁。
...
## 配置契约课堂：哪些值必须现在知道，哪些可以以后知道

### 稳定键与未知属性可以共存

`for_each` 需要在规划时确定实例身份：映射的键，或者字符串集合中的成员。映射的某些值却可以是创建后才知道的属性。老师让你建两台机器，键叫 `api` 和 `worker`，它们的网络地址稍后由平台分配，这样身份已知而属性未知是合理的；若把尚未分配的地址本身当键，工具就不能提前列出要管理哪些实例。[for_each 的已知值要求](https://developer.hashicorp.com/terraform/language/meta-arguments/for_each)

集合还会去重，而且不应依赖原列表顺序。把三个不同业务对象转换成包含两个重复名称的集合，可能只得到两个实例；这不是调度漏建，而是输入已经表达了两个唯一身份。名字必须承担业务身份时，要在输入验证阶段拒绝重复或冲突，而不是等待 apply 后才数机器。敏感信息也不适合当实例键，因为地址和界面会展示键，隐藏一个变量并不能让它安全成为公开身份。

`null`、空集合与未知值同样不同。空集合表示现在知道不需要任何实例，可能让此前受管实例进入删除计划；未知值表示暂时无法确定数量或身份，通常无法在这种位置完成规划。看到上游查询为空，必须先判断是真实业务清空还是查询与权限异常。不能把异常返回转换成空集合，再让工具“自动收敛”删除全部资源。

### 文件读取不自动变成等待文件生成

Terraform 表达式函数通常用于计算输入，不是部署步骤。例如 `file()` 读取已有文件，不能因为另一个资源稍后会创建这个路径，就假设函数自动等待它生成。函数本身不参与资源依赖图。需要资源创建结果时，应优先引用资源导出的属性，或采用适合该数据流的受支持数据源；不要用同一路径字符串冒充依赖关系。[表达式与依赖关系](https://developer.hashicorp.com/terraform/language/expressions/references)

这也解释了为什么在配置里到处加 `depends_on` 不一定解决问题。依赖图可以控制资源操作顺序，却不能把所有读取函数改成运行时任务。排查先看值是在配置求值、计划还是执行阶段产生，再选接口。把构建产物先生成并验证，再作为固定输入交给基础设施计划，通常比在一次复杂 apply 里混入随意构建脚本更容易审计。

### 检查失败到底是阻断还是提醒

变量校验约束输入，例如允许的环境或数值范围；资源前置条件检查开始操作所需假设，后置条件检查得到的结果是否满足约定。它们适合让配置契约更清楚，但前置条件仍不能替代云端权限：攻击者换一个执行入口，不会因为这里写了条件就失去账户权限。[生命周期条件](https://developer.hashicorp.com/terraform/language/meta-arguments/lifecycle)

`check` 块用于持续性断言，其失败通常产生警告而非阻止操作；不能把必须阻断生产的条件随意搬进 check 后就认为仍是强门禁。团队应明确哪一层负责阻断，CI 又如何处理警告。验证错误提示应解释具体输入和期待范围，不输出敏感值。实际资源已改变后才出现后置条件失败，也不代表 Terraform 自动撤销刚才的操作，仍要按部分失败恢复。

### 后端锁与云资源凭据是两条认证链

S3 backend 读取状态对象所需的身份，与 AWS provider 创建业务资源所需的身份可以分别配置。一个账号能读状态不代表能修改服务器；能够改服务器也不代表能写回状态。排查 init、plan 和 apply 的认证错误要看是哪条连接失败，不能直接把所有凭据改成管理员以求通过。

前面的 S3 最小示例仅说明存储位置，未启用锁。使用支持的 Terraform 版本时可按当前文档配置基于 S3 锁文件的机制，并授予状态对象与锁对象各自需要的最小权限；较旧环境的锁方案不同，迁移前核验版本，不把示例当成已完成并发保护。对象版本控制有助于恢复旧状态，但恢复仍需与真实资源核对，不能盲目选一个较早对象覆盖当前映射。[S3 后端锁与权限](https://developer.hashicorp.com/terraform/language/backend/s3)

最后还要识别计划退出码。普通计划成功未必告诉自动化是否有差异；使用 `-detailed-exitcode` 时，零表示成功且无差异，一表示错误，二表示成功但存在差异。接入持续检查时应显式处理这三类，不能让开启遇错即停的脚本把二误当成工具损坏，也不能把一当成“没有变更”。保存退出码后再进行分支判断，日志记录阶段和脱敏计划摘要。这个约定只用于对应命令选项，不能推广为所有 Terraform 命令通用的退出码含义。

### 不执行云操作的身份实验

前置条件是本地文件实验已跑通、学习目录只管理自己的文件。在笔记写出映射键 `api`、`worker`，地址值均未知，预测有两个已知实例身份；再把地址作为集合成员，预测无法确定实例身份；最后把输入改为空集合，预测期望实例数为零。这个练习只推理数据模型，不需要修改已经受管的配置。

故障注入是把一次外部查询失败错误地记成空集合。预期审阅应阻断，因为“查询失败”不能等同“批准删除全部对象”。修复是让查询错误保持显式失败，并从可信资产清单确定期望集合。验证时分别列输入证据、计划身份与可能销毁影响；若三个样例都被描述成同一个未知状态，重新比较空值和未知值的含义。清理删除合成笔记，无云资源或 state 需要恢复。

三十秒追问答案：Terraform 先需要确定资源身份，再允许部分属性在执行后获知；输入缺失、空集合和错误不能混用。三分钟把数据来源、校验、依赖图、状态锁、执行身份和业务验收串起来。架构题“资产接口暂时返回空数据”要先防止错误清空期望集合，再讨论缓存与人工确认，不能让自动化把观测故障扩大为大规模销毁。

把差异检测与执行授权分开记录：检测到差异只创建审阅任务，不自动代表允许修改。尤其是销毁或替换计划，需要资源所有者确认数据影响和恢复条件。
