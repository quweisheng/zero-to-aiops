...
| 第 2 行 | <code>  -&gt; terraform init 下载 provider</code> | 这一行要理解这些英文词：`terraform init` 是terraform=基础设施即代码工具，init=初始化；`provider` 是Terraform 连接云厂商或平台的插件。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 3 行 | <code>  -&gt; terraform validate / fmt</code> | 这一行要理解这些英文词：`terraform validate` 是terraform=基础设施即代码工具，validate=校验；`fmt` 是格式化。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 4 行 | <code>  -&gt; terraform plan 读取 state 和真实资源</code> | 这一行要理解这些英文词：`terraform plan` 是terraform=基础设施即代码工具，plan=执行计划；`state` 是状态。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 5 行 | <code>  -&gt; execution plan</code> | 这一行要理解这些英文词：`execution plan` 是执行计划，Terraform 或数据库在真正执行前给出的操作方案。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 6 行 | <code>  -&gt; terraform apply 调 provider API</code> | 这一行要理解这些英文词：`terraform apply` 是terraform=基础设施即代码工具，apply=应用变更；`provider API` 是provider=Terraform 连接云厂商或平台的插件，api=应用程序接口。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 7 行 | <code>  -&gt; 创建/修改/删除真实资源</code> | 这一行表示上一级主题下的子项“创建/修改/删除真实资源”。`->` 只是知识地图里的层级符号，真正要理解的是这句话里的操作或概念。 |
| 第 8 行 | <code>  -&gt; 更新 terraform.tfstate 或 remote state</code> | 这一行要理解这些英文词：`terraform.tfstate` 是terraform=基础设施即代码工具；`remote state` 是state=状态。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 第 2 行 | <code>  -&gt; init</code> | 这一行要理解这些英文词：`init` 是初始化。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 3 行 | <code>  -&gt; validate</code> | 这一行要理解这些英文词：`validate` 是校验。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 4 行 | <code>  -&gt; fmt</code> | 这一行要理解这些英文词：`fmt` 是格式化。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 5 行 | <code>  -&gt; plan</code> | 这一行要理解这些英文词：`plan` 是执行计划。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 6 行 | <code>  -&gt; apply</code> | 这一行要理解这些英文词：`apply` 是应用变更。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 7 行 | <code>  -&gt; destroy</code> | 这一行要理解这些英文词：`destroy` 是销毁资源，Terraform 中通常表示删除已创建的基础设施。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 8 行 | <code>  -&gt; output</code> | 这一行要理解这些英文词：`output` 是输出。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 9 行 | <code>  -&gt; show</code> | 这一行要理解这些英文词：`show` 是显示信息，常用于查看状态、资源或执行计划。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 10 行 | <code>  -&gt; state</code> | 这一行要理解这些英文词：`state` 是状态。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 11 行 | <code>  -&gt; import</code> | 这一行要理解这些英文词：`import` 是导入已有资源。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 12 行 | <code>  -&gt; providers</code> | 这一行要理解这些英文词：`providers` 是Terraform Provider 插件。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 13 行 | <code>  -&gt; workspace</code> | 这一行要理解这些英文词：`workspace` 是工作区。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 14 行 | <em>空行</em> | 空行，用来把示例结构分成更容易阅读的段落。 |
| 第 15 行 | <code>Terraform Language</code> | 文本示例行，用来展示输出、目录、流程、错误信息或学习证据中的一条内容。 |
| 第 16 行 | <code>  -&gt; configuration syntax</code> | 这一行要理解这些英文词：`configuration syntax` 是configuration=配置。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 17 行 | <code>  -&gt; terraform block</code> | 这一行要理解这些英文词：`terraform block` 是Terraform 块，声明 Terraform 版本、Provider 版本等全局要求。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 18 行 | <code>  -&gt; provider block</code> | 这一行要理解这些英文词：`provider block` 是Provider 块，声明 Terraform 要连接哪个云厂商或平台。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 19 行 | <code>  -&gt; resource block</code> | 这一行要理解这些英文词：`resource block` 是资源块，声明要创建或管理的基础设施对象。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 20 行 | <code>  -&gt; data block</code> | 这一行要理解这些英文词：`data block` 是数据块，Terraform 中读取外部已有资源信息的配置块。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 21 行 | <code>  -&gt; variable block</code> | 这一行要理解这些英文词：`variable block` 是变量块，声明 Terraform 可输入参数。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 22 行 | <code>  -&gt; output block</code> | 这一行要理解这些英文词：`output block` 是output=输出，block=配置块。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 23 行 | <code>  -&gt; locals</code> | 这一行要理解这些英文词：`locals` 是本地变量，Terraform 中用于保存中间表达式结果。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 24 行 | <code>  -&gt; modules</code> | 这一行要理解这些英文词：`modules` 是模块。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 25 行 | <code>  -&gt; expressions</code> | 这一行要理解这些英文词：`expressions` 是表达式，由变量、函数和运算符组成的计算语句。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 26 行 | <code>  -&gt; functions</code> | 这一行要理解这些英文词：`functions` 是函数。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 27 行 | <code>  -&gt; meta-arguments</code> | 这一行要理解这些英文词：`meta-arguments` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 28 行 | <em>空行</em> | 空行，用来把示例结构分成更容易阅读的段落。 |
| 第 29 行 | <code>State</code> | 文本示例行，用来展示输出、目录、流程、错误信息或学习证据中的一条内容。 |
| 第 30 行 | <code>  -&gt; state file</code> | 这一行要理解这些英文词：`state file` 是状态文件，Terraform 用它记录代码和真实资源的对应关系。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 31 行 | <code>  -&gt; resource addressing</code> | 这一行要理解这些英文词：`resource addressing` 是资源地址，Terraform 用来唯一定位某个资源的名字。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 32 行 | <code>  -&gt; state locking</code> | 这一行要理解这些英文词：`state locking` 是状态锁，防止多人同时修改同一份 Terraform 状态。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 33 行 | <code>  -&gt; backends</code> | 这一行要理解这些英文词：`backends` 是后端状态存储。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 34 行 | <code>  -&gt; refresh</code> | 这一行要理解这些英文词：`refresh` 是刷新状态，重新读取真实资源或最新数据。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 35 行 | <code>  -&gt; drift</code> | 这一行要理解这些英文词：`drift` 是漂移，真实资源和代码不一致。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 36 行 | <code>  -&gt; import</code> | 这一行要理解这些英文词：`import` 是导入已有资源。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 37 行 | <em>空行</em> | 空行，用来把示例结构分成更容易阅读的段落。 |
| 第 38 行 | <code>Providers</code> | 文本示例行，用来展示输出、目录、流程、错误信息或学习证据中的一条内容。 |
| 第 39 行 | <code>  -&gt; provider source</code> | 这一行要理解这些英文词：`provider source` 是provider=Terraform 连接云厂商或平台的插件，source=来源。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 40 行 | <code>  -&gt; provider version</code> | 这一行要理解这些英文词：`provider version` 是provider=Terraform 连接云厂商或平台的插件。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 41 行 | <code>  -&gt; provider configuration</code> | 这一行要理解这些英文词：`provider configuration` 是Provider 配置，设置区域、账号、端点等连接参数。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 42 行 | <code>  -&gt; provider authentication</code> | 这一行要理解这些英文词：`provider authentication` 是Provider 认证，Terraform 调云厂商或平台 API 前的登录凭据。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 43 行 | <code>  -&gt; provider registry docs</code> | 这一行要理解这些英文词：`provider registry docs` 是Provider Registry 文档，查询资源类型和字段的官方说明。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 44 行 | <em>空行</em> | 空行，用来把示例结构分成更容易阅读的段落。 |
| 第 45 行 | <code>Modules</code> | 文本示例行，用来展示输出、目录、流程、错误信息或学习证据中的一条内容。 |
| 第 46 行 | <code>  -&gt; root module</code> | 这一行要理解这些英文词：`root module` 是根模块，当前 Terraform 项目的入口模块。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 47 行 | <code>  -&gt; child module</code> | 这一行要理解这些英文词：`child module` 是module=模块。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 48 行 | <code>  -&gt; module source</code> | 这一行要理解这些英文词：`module source` 是module=模块，source=来源。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 49 行 | <code>  -&gt; inputs</code> | 这一行要理解这些英文词：`inputs` 是输入值。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 50 行 | <code>  -&gt; outputs</code> | 这一行要理解这些英文词：`outputs` 是输出值。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 51 行 | <code>  -&gt; versioning</code> | 这一行要理解这些英文词：`versioning` 是版本管理。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 第 2 行 | <code>  -&gt; 再懂 state 和 plan</code> | 这一行要理解这些英文词：`state` 是状态；`plan` 是执行计划。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 3 行 | <code>  -&gt; 再懂 variables/outputs</code> | 这一行要理解这些英文词：`variables` 是变量；`outputs` 是输出值。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 4 行 | <code>  -&gt; 再懂 modules</code> | 这一行要理解这些英文词：`modules` 是模块。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 5 行 | <code>  -&gt; 再懂 backend/locking</code> | 这一行要理解这些英文词：`backend` 是后端状态存储；`locking` 是加锁。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 6 行 | <code>  -&gt; 最后学 import、drift、lifecycle 和团队协作</code> | 这一行要理解这些英文词：`import` 是导入已有资源；`drift` 是漂移，真实资源和代码不一致；`lifecycle` 是生命周期控制。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 第 2 行 | <code>  -&gt; Terraform configuration</code> | 这一行要理解这些英文词：`Terraform configuration` 是terraform=基础设施即代码工具，configuration=配置。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 3 行 | <code>  -&gt; CI plan</code> | 这一行要理解这些英文词：`CI plan` 是ci=持续集成，plan=执行计划。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 4 行 | <code>  -&gt; 人工审批</code> | 这一行表示上一级主题下的子项“人工审批”。`->` 只是知识地图里的层级符号，真正要理解的是这句话里的操作或概念。 |
| 第 5 行 | <code>  -&gt; terraform apply</code> | 这一行要理解这些英文词：`terraform apply` 是terraform=基础设施即代码工具，apply=应用变更。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 6 行 | <code>  -&gt; 云资源 / Kubernetes / DNS / IAM / 监控资源</code> | 这一行要理解这些英文词：`Kubernetes` 是容器编排平台，用来部署、调度和管理容器化应用；`DNS` 是域名解析系统，把域名转换成 IP 地址；`IAM` 是身份与访问管理，用来控制账号、角色和权限。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 7 行 | <code>  -&gt; Prometheus / Grafana / Alertmanager 观测</code> | 这一行要理解这些英文词：`Prometheus` 是指标监控系统；`Grafana` 是仪表盘和可视化平台，用来展示指标、日志和告警数据；`Alertmanager` 是Prometheus 生态里的告警管理器。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 8 行 | <code>  -&gt; drift detection / change correlation</code> | 这一行要理解这些英文词：`drift detection` 是drift=漂移，真实资源和代码不一致，detection=发现或检测；`change correlation` 是change=变更。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 行 | 内容 | 说明 |
|---|---|---|
| 第 1 行 | <code>配置 + 状态 + Provider API -&gt; 差异计划 -&gt; 执行并更新状态</code> | 这一行要理解这些英文词：`Provider API` 是provider=Terraform 连接云厂商或平台的插件，api=应用程序接口。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
