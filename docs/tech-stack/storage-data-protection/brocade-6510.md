...
```text
物理光链路建立
  -> FLOGI：设备登录 Fabric，获得 FC Address（交换网络内地址）
  -> PLOGI：Initiator 与 Target（发起端与目标端）建立端到端登录
  -> PRLI：协商上层协议角色，例如 FCP（在 FC 上传输 SCSI）的发起端/目标端
  -> SCSI 命令和数据帧开始传输
```
...
使用脱敏 CSV 模拟 `porterrshow` 与端口台账，检查端口状态、CRC、C3 discard 和 loss of sync。它只覆盖预设端口，不证明完整端到端路径冗余；多路径验证见后文进阶课堂。
...
problems = []
expected = {("A", "san-a-01", "0"), ("A", "san-a-01", "8"),
            ("B", "san-b-01", "0"), ("B", "san-b-01", "8")}
seen = set()

with open("fabric-ports.csv", encoding="utf-8", newline="") as file:
    for row in csv.DictReader(file):
        identity = (row["fabric"], row["switch"], row["port"])
        if identity in seen or identity not in expected:
            raise ValueError("重复或未知的课堂端口，采集结果不能判为健康")
        seen.add(identity)
...
if seen != expected:
    raise ValueError("课堂端口覆盖不完整，采集结果不能判为健康")
...
1. 确认当前目录是本轮独立实验目录且 `fabric-ports.backup.csv` 不存在，再执行 `Copy-Item .\fabric-ports.csv .\fabric-ports.backup.csv`；已有备份则停止，不覆盖。
...
## 端到端课堂：交换机看见四个口，不等于主机有四条可用路径

一条主机存储路径至少包含发起端、目标端和逻辑设备身份。交换机端口在线只是其中的物理条件，名称服务、登录、生效 zoning、阵列呈现与主机多路径识别还要继续成立。基础实验的四条 CSV 是四个被监测端口，不是四条已经验证的主机到卷路径。因此脚本只能给出端口样例的健康判断，不能用于签署生产双 Fabric 验收。

建立进阶台账时，每行写主机、HBA WWPN、Fabric、交换机端口、目标 WWPN、阵列设备标识和主机路径状态。主机看到多个操作系统设备名时，应以受支持的多路径设备标识确认它们指向同一个卷。设备名会随发现次序变化，不适合作为唯一身份；误把两条路径当两块新盘初始化，可能直接破坏原业务数据。

NPIV 是一个物理端口承载多个虚拟端口身份的机制。一个交换机接入口可能对应多个虚拟 WWPN，而不是一对一映射到一台虚拟机。排查虚拟机迁移后的路径变化，应核对虚拟 WWPN、实际登录位置和平台的迁移方式，不因为物理口没变就断言业务身份没变。也不能把父端口在线理解为所有虚拟端口都登录成功。

基础纸面实验列出主机甲的两条路径：甲一经 A 到目标一，甲二经 B 到目标二，并标记同一卷身份。故障步骤将第二条路径的目标替成另一个卷，端口仍全部在线。预期端口检查通过但路径一致性检查失败，正确处置是阻止错误卷写入并核对呈现。恢复步骤还原目标与卷关系，再由主机层验证；清理只删除虚构表，不修改真实 zoning。

## 信用课堂：带宽空闲也可能被传播时延限制

信用控制的是逐跳在途帧，不是应用层一次数据库事务的完成通知。发出帧时扣除信用，接收端释放相应接收资源后返还；信用足够前，发送方必须等待。这和 TCP 窗口、主机命令队列有关联但并不相同：加大主机队列不会自动增加链路接收缓存，也不能修复已经不返还信用的终端。

用理想化模型理解长距离：若可同时在途的总帧数据量为十六 KiB，信用周转需要一毫秒，那么仅由这两个条件给出的吞吐上限约为每秒十六 MiB 的数量级；它不是某款端口的实际规格，忽略了编码、控制帧和处理开销。距离增加使周转变慢，即使端口标称速率很高，发送者仍可能经常等信用。真实配置使用厂家计算方法、帧长、距离与许可证，不能照这个课堂数字调生产端口。

慢排水与长距离不足可能都表现为信用等待，但证据不一样。距离和路径稳定时，突然出现的等待更需要检查终端处理与负载变化；新增长距离链路从上线就达不到预期，则需要检查链路预算和信用配置。两者还可能同时存在。先比较同方向相邻端口与不经过故障目标的流，缩小候选原因后才提出配置变化。

Broadcom 的 [SANnav 故障模式说明](https://docs.broadcom.com/htmldocs/SANnav111/SANnav111/v24046718.html)把帧超时和发送信用为零作为性能影响分析线索。这些线索需要结合具体平台与 FOS；新代际自动隔离或低优先级通道能力不能直接推定旧 6510 可用。即使有授权的自动动作，也应先确认另一 Fabric 真能接管，才能允许执行隔离。

## 光链路课堂：接收错误的位置不一定是损坏组件的位置

假设交换机端口收到坏帧并统计 CRC，可能原因在对端发光、光纤、连接器、本端接收或更上游已经损坏的帧处理，需要依据平台具体计数语义继续定位。看到本端错误，不等于本端交换机必坏。记录发送方向、接收方向和两端计数，避免把一条双向链路误当成只有一个方向的管道。

基础步骤在虚构接线图上标出主机发送到交换机和交换机发送到主机两条方向，分别放置接收光功率与错误增量。故障步骤只降低主机到交换机这一方向的接收质量，预测另一个方向的读数可以正常。恢复步骤替换单个已识别变量并比较同一采样长度的增量。核验应包含相同负载下错误停止增长，清理仅恢复模拟数值。

真实换线还要考虑停留窗口。先确认业务路径、批准操作、光纤标签与清洁规范，不对着一排相似跳纤试拔。旧件隔离标记并保留回退位置，每次改变一个变量。换完后短时间没有流量，错误自然也可能不增加；必须结合有效业务或批准的测试负载，不能把零增量直接当成修复证明。

## Zoning 变更课堂：保存配置、启用配置与业务生效是三个关口

Defined 表示已定义对象，Effective 表示当前生效集合。把新 zone 写入定义但没有进入正确生效配置，主机仍可能无法访问；启用了错误配置，则可能影响远多于本次申请的主机。每次变更前需要比较完整生效集合，而不是只检查新增的一行是否正确。

把配置当作共享数据库理解更安全：两个人同时基于旧基线修改，各自认为只加了一条，但最终启用的集合可能遗漏另一人的更新。具体事务、锁定和保存行为依 FOS 版本，在维护窗口规定唯一变更者、变更前后差异和并发停止条件。不能因为 CLI 能接受命令就忽略其他人正在操作同一个 Fabric。

纸面故障题准备三个原有 zone 和一个新 zone。基础步骤得到四条的候选配置，故障步骤故意漏掉一个原有 zone，要求从成员集合差异发现潜在影响。恢复步骤补齐原成员并由另一人按 WWPN 复核。预期业务范围只增加目标主机而没有其他删除；清理删除候选文本，保留权威原配置。不在真实设备执行 `cfgenable` 来验证一个集合运算。

回滚也需要确认期间有没有合法新增业务。直接恢复几天前的整份配置，可能删掉故障之后其他团队已经验收的访问。维护流程应冻结并发或逐项合并所需差异，记录谁认定哪份配置具有权威性。对未知 zone 不随意删除，对看似相同的 Alias 仍核对真实 WWPN，不能只凭命名风格判断归属。

## 迁移设计追问：备件、授权和观察窗口也是可用性

从旧交换机搬到新交换机，需要的不只是可用物理口。端口激活许可、互联聚合许可、距离能力、兼容光模块、目标 FOS、阵列和 HBA 支持组合都要在 BOM 中逐项确认。BOM 是物料及组件清单；“机箱有四十八个孔”不代表四十八口都已激活，也不代表旧光模块能直接复用。供应合同应说明实际交付和合法固件来源。

假设某批主机原来有八条路径，迁移后仍有四条，看起来业务正常，但四条全经同一个目标端口。验收必须比较故障域，而不是只设置最低数量。再问失去一个 ISL 后是否超过容量，备份与批处理高峰是否会挤占业务，只有日常低负载测试不能证明故障后的峰值承载能力。

退出旧 Fabric 的时间也要有依据。保留期内旧配置、接线和合法许可可用，才能按方案回接；若提前清除配置或挪走模块，所谓回滚图已经失效。反之无限保留旧设备会增加管理面和误接风险。约定观察期、业务签字、证据归档和安全退役顺序，让回退能力与最终清理都有明确终点。

离线实验的输入完整度同样影响结论。本文检查器要求四个预设端口齐全且不重复；空文件、少一行、未知端口会产生异常而非健康。正常样例恢复后应重新看到零退出码，再删除本轮备份。若只做基础课，可保留 CSV 与脚本作为作业；如需完全回收，在确认独立目录内没有其他文件后只删除自己创建的文件，再删除空目录，不递归清理上级路径。本轮只做文档和离线模型核对，没有连接交换机、清零计数或切换路径。

### 容量练习：端口总带宽相加为什么会误导

假设四台主机各有一条十六 G 连接，但跨交换机只有一条十六 G 互联。把接入口相加得到六十四 G，并不代表跨交换机可以承诺六十四 G；同时访问另一侧目标时会共享互联。实际有效吞吐还要扣除协议开销，流量方向与是否经过互联也必须明确。目标在本地交换机时，不应把这部分流量重复计入跨交换机负载。

基础步骤为四台主机分别画目标位置与峰值流量，只把确实经过互联的流相加。故障步骤让两条互联中的一条不可用，重新计算剩余承载；恢复选择可能是推迟备份、减少非关键批量任务或增加受支持链路，不是把所有主机端口降速。预期结果是一张正常、单链路失效和恢复后的容量表，清理只删除课堂流量数据。

指标的单位也要一致。速率里的位与监控里的字节不能直接相加，每秒与五分钟累计量不能直接比较；厂商图表采用十进制还是二进制单位也应写清。发现表里“峰值超过端口物理能力”时，先检查聚合对象、采样范围与单位，再讨论异常硬件。采集多个交换机的同一条流量后直接求和，还可能把同一帧重复计算。

本轮对文中 Python 检查器实际做了内存输入验证：正常样例退出零、故障样例退出二、空输入和重复端口退出一。输入通过内存替代文件读取，没有新建实验目录或连接设备；这证明课堂判断分支可执行，不证明 FOS 输出解析或真实路径切换已经通过。

采样失败与设备故障分开记账：前者先查采集权限和连接，后者再按路径证据定位，不能把无法登录直接写成光链路中断。
