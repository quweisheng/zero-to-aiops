...
老师给你一张变更单：“今晚升级阵列，失败就退回旧版本。”这句话为什么还不能批准？因为软件代码、卷数据、复制角色、主机路径和配置元数据是不同对象。恢复一份旧配置不会自动恢复升级后写入的数据；把旧安装包传回设备，也不代表系统支持原地降级。必须先按具体型号和当前代码查允许路径、测试工具、已知问题与厂商支持条件，不能从另一款 IBM 产品的经验直接推出结论。[IBM 升级规划](https://www.ibm.com/support/pages/ibm-storage-virtualize-family-products-upgrade-planning)提供入口，但实际变更还需要现场物料清单与兼容矩阵。

物料清单不只列阵列型号，还要列控制器、适配卡、磁盘类型、交换机代码、主机系统、多路径软件和所用复制功能。功能已经出现在产品宣传页，不等于现场版本支持或已经购买相应授权。把“支持”“已授权”“已启用”“已验证”分成四项，才能避免上线当天发现灾备复制不可用。对于依赖旧复制方式的环境，应单独核对目标版本的限制，不在教材中承诺任意跨版本直升。

设计停止条件时，用业务证据约束下一步：第一个节点维护后，如果某主机只剩零条路径、卷响应时间持续超出约定范围，或复制状态进入不允许继续的状态，就停止后续节点操作并按已批准流程升级支持。不要等两个节点都处理完再看业务。回退可能是恢复原路径、暂停升级、切回业务访问入口，或在厂商指导下执行受支持恢复；它未必是软件降级。每个选项都应写明数据去向、剩余冗余和操作者权限。

把这段变成桌面演练：准备一张虚构表，主机甲两条路径分别经过两个 Fabric，主机乙两条都经过同一 Fabric。正常轮统计都为两条；故障轮划掉该 Fabric，预期主机甲仍可访问、主机乙中断。验证应检查独立故障域，不是仅检查路径数量。恢复轮把主机乙设计为真正双 Fabric，并重新逐条追踪到控制器；清理只擦除纸面故障标记，不实际禁用任何端口。若推演得出“双控保证所有主机不中断”，说明遗漏了主机到阵列的中间层。

前面生成健康报告的基础实验，开始前还应确认实验目录不含自己的既有文件；若已有同名样本就换一个独立目录。结束后可保留两份脱敏 CSV 作为学习证据，或在文件管理器中只删除本轮生成的 `metrics.csv` 与 `health-report.csv`，不递归清理上级目录。本轮文档补强没有连接 IBM 设备、执行升级或验证真实故障切换，桌面演练只能证明推理链，不能代替设备恢复验收。
