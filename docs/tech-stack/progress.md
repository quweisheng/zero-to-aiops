...
## 2026-09-10 全部 114 篇完成本轮老师带学修订

在前批 34 篇达到万字正文的基础上，本轮继续补强其余 80 篇，并复核已有文章。现在全部 114 篇中文正文均不少于 10,000 字；统计排除代码、命令输出和文字图，保留原有较长的有效解释。最新逐篇数值由 [篇幅复核表](./teaching-coverage.md) 从实际内容生成。

本轮按业务场景补前置概念、术语与中文知识地图、请求和数据路径、配置含义、基础与故障实验、排障证据、架构取舍及递进面试回答。基础工具、前端、云原生、私有云、存储、数据库、AI、自动化、可观测性和 SRE 专题使用各自的技术语境，不用统一套话凑字数。路线、资料导读和项目模板继续衔接独立练习与 GitHub 学习证据。

同时修正实验中发现的输入缺失误判、旧报告覆盖、清理范围、时间戳、查询语义、标签与告警作用域、版本和产品边界等问题。课堂合成模型、语法检查与本地接口契约测试只证明各自检查范围；没有在本轮启动全部真实产品或验证生产高可用、数据恢复和性能。读者仍需按各篇条件在隔离环境实跑，不能把“预期结果”当成自己的项目经历。

后续新产品、版本变化或读者反馈属于持续维护，不再把已经完成的本轮 114 篇目标写成待补齐。下方保留历史批次记录，便于理解知识库的演进，当前状态以本节及复核表为准。

## 历史批次：2026-09-10 数据、可观测与事故处理
...
[事故证据与运行手册课堂](https://github.com/quweisheng/zero-to-aiops/tree/main/examples/incident-evidence-classroom) 使用合成统计和内存状态模拟，练习流量构成导致的汇总变化、缺失证据保护、丢响应后的结果核对，以及计划和版本漂移拦截。它不连接真实服务、不证明持久性或生产高可用；各组件容器实验仍需读者在隔离环境实跑并提交证据。其他技术栈在该批结束时仍有篇幅差距，后续已在上方全站修订中补齐。

## 2026-09 老师带学全站修订

本轮范围是全部 114 篇技术栈，以及路线、能力地图、面试导读、岗位路线、资料导读和项目/周复盘模板。统一增加中文地图释义、前置概念、课堂推理、正常/故障练习和分层面试表达，并修订发现的技术或示例问题。

历史表中的“第一版/第二版”是当时批次状态，不用于判断当前篇幅是否达标。最新实际字数和复核方向见 [老师带学修订与篇幅复核](./teaching-coverage.md)。自动扫描仅提示结构线索，不能替代内容审校或真实环境实验。

新增 [可靠性与可观测性课堂实验](https://github.com/quweisheng/zero-to-aiops/tree/main/examples/teacher-led-reliability-lab) 使用合成数据，分别练习预算、分组、时间线、因果证据、幂等、数据泄漏、指标类型、单位和链路关联。它验证算术和判断流程；真实平台的安装、连接、高可用与恢复仍按各篇说明独立演练并留下实际记录。
...
| 已达到大厂面试级第一版 | IBM WebSphere | 已补 traditional ND 与 Liberty 边界、请求与配置同步路径、EAR 更新、BLA/CU 残留、集群、会话、JDBC/JMS/JTA、容量、高可用、安全、升级回滚、双层实验、事故题和系统设计 |
...
| 已达到大厂面试级第一版 | PyTorch | [pytorch.md](./data-ai/pytorch.md) | PyTorch 2.13、Tensor/Autograd/Module、Dataset/DataLoader、状态与恢复、AMP/compile、DDP/FSDP2、容量安全、服务观测、CPU 基础实验、NaN 故障注入、事故题和系统设计 |
...
| 20 | IBM WebSphere | 大厂面试级第一版已完成 | 新增中间件与应用平台分类，补齐 traditional ND 与 Liberty、请求与管理路径、EAR 更新与 Composition Unit 残留、集群和状态、容量安全、双层实验、事故复盘和生产系统设计 |
...
| 数据与 AI | PyTorch | [pytorch.md](./data-ai/pytorch.md) |
