...
| 第 2 行 | <code>  -&gt; DNS 解析出 IP</code> | 这一行要理解这些英文词：`DNS` 是域名解析系统，把域名转换成 IP 地址；`IP` 是网络地址，用来定位一台机器或一个服务入口。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 3 行 | <code>  -&gt; 本机路由选择网卡和网关</code> | 这一行表示上一级主题下的子项“本机路由选择网卡和网关”。`->` 只是知识地图里的层级符号，真正要理解的是这句话里的操作或概念。 |
| 第 4 行 | <code>  -&gt; TCP 连接目标 IP:端口</code> | 这一行要理解这些英文词：`TCP` 是传输控制协议，提供可靠的网络连接；`IP` 是网络地址，用来定位一台机器或一个服务入口。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 5 行 | <code>  -&gt; TLS 校验证书并协商加密</code> | 这一行要理解这些英文词：`TLS` 是加密传输协议，保护 HTTPS 等连接的安全。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 6 行 | <code>  -&gt; HTTP 发送请求</code> | 这一行要理解这些英文词：`HTTP` 是超文本传输协议。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 第 3 行 | <code>  -&gt; DNS: RFC 1034 / RFC 1035 / RFC 8499</code> | 这一行要理解这些英文词：`DNS` 是域名解析系统，把域名转换成 IP 地址；`RFC` 是互联网标准文档，很多协议和最佳实践会用 RFC 定义。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 4 行 | <code>  -&gt; TCP: RFC 9293</code> | 这一行要理解这些英文词：`TCP` 是传输控制协议，提供可靠的网络连接；`RFC` 是互联网标准文档，很多协议和最佳实践会用 RFC 定义。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 5 行 | <code>  -&gt; TLS: RFC 8446</code> | 这一行要理解这些英文词：`TLS` 是加密传输协议，保护 HTTPS 等连接的安全；`RFC` 是互联网标准文档，很多协议和最佳实践会用 RFC 定义。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 6 行 | <code>  -&gt; HTTP: RFC 9110 / RFC 9111 / RFC 9112 / RFC 9113 / RFC 9114</code> | 这一行要理解这些英文词：`HTTP` 是超文本传输协议；`RFC` 是互联网标准文档，很多协议和最佳实践会用 RFC 定义。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 第 10 行 | <code>  -&gt; ip(8): 查看和修改地址、链路、路由</code> | 这一行要理解这些英文词：`ip` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 11 行 | <code>  -&gt; ip-route(8): 路由表</code> | 这一行要理解这些英文词：`ip-route` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 12 行 | <code>  -&gt; resolv.conf(5): DNS resolver 配置</code> | 这一行要理解这些英文词：`resolv.conf` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题；`DNS resolver` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 13 行 | <code>  -&gt; getaddrinfo(3): 应用如何把名字解析成地址</code> | 这一行要理解这些英文词：`getaddrinfo` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 14 行 | <em>空行</em> | 空行，用来把示例结构分成更容易阅读的段落。 |
| 第 15 行 | <code>工具官方文档</code> | 文本示例行，用来展示输出、目录、流程、错误信息或学习证据中的一条内容。 |
| 第 16 行 | <code>  -&gt; curl: HTTP/TLS/DNS/代理/超时诊断</code> | 这一行要理解这些英文词：`curl` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题；`HTTP` 是超文本传输协议；`TLS` 是加密传输协议，保护 HTTPS 等连接的安全；`DNS` 是域名解析系统，把域名转换成 IP 地址。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 17 行 | <code>  -&gt; OpenSSL s_client: TLS 证书和握手诊断</code> | 这一行要理解这些英文词：`OpenSSL s_client` 是client=客户端；`TLS` 是加密传输协议，保护 HTTPS 等连接的安全。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 18 行 | <em>空行</em> | 空行，用来把示例结构分成更容易阅读的段落。 |
| 第 19 行 | <code>发行版和云厂商文档</code> | 文本示例行，用来展示输出、目录、流程、错误信息或学习证据中的一条内容。 |
| 第 20 行 | <code>  -&gt; NetworkManager、systemd-resolved、防火墙、负载均衡、VPC</code> | 这一行要理解这些英文词：`NetworkManager` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题；`systemd-resolved` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题；`VPC` 是英文缩写或固定标识，结合本节上下文记住它代表的组件、命令或状态。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 第 5 行 | <code>  -&gt; NGINX / Ingress</code> | 这一行要理解这些英文词：`NGINX` 是高性能 Web 服务器和反向代理，常用于网关、转发和负载均衡；`Ingress` 是Kubernetes 入站流量规则，定义外部请求如何进入集群服务。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 6 行 | <code>  -&gt; Service / Pod / VM</code> | 这一行要理解这些英文词：`Service` 是服务；`Pod` 是Kubernetes 最小调度单元，里面可以运行一个或多个容器；`VM` 是英文缩写或固定标识，结合本节上下文记住它代表的组件、命令或状态。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 第 11 行 | <code>  -&gt; exporter 暴露指标</code> | 这一行要理解这些英文词：`exporter` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 12 行 | <code>  -&gt; Prometheus 抓取</code> | 这一行要理解这些英文词：`Prometheus` 是指标监控系统。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 13 行 | <code>  -&gt; Alertmanager 发告警</code> | 这一行要理解这些英文词：`Alertmanager` 是Prometheus 生态里的告警管理器。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 14 行 | <code>  -&gt; Grafana 展示</code> | 这一行要理解这些英文词：`Grafana` 是仪表盘和可视化平台，用来展示指标、日志和告警数据。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 15 行 | <code>  -&gt; Runbook 自动化调用 HTTP / SSH / API</code> | 这一行要理解这些英文词：`Runbook` 是故障处理手册；`HTTP` 是超文本传输协议；`SSH` 是远程登录协议；`API` 是应用程序接口。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 第 2 行 | <code>  -&gt; DNS resolver</code> | 这一行要理解这些英文词：`DNS resolver` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 3 行 | <code>  -&gt; A / AAAA 记录</code> | 这一行要理解这些英文词：`A` 是英文缩写或固定标识，结合本节上下文记住它代表的组件、命令或状态；`AAAA` 是英文缩写或固定标识，结合本节上下文记住它代表的组件、命令或状态。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 4 行 | <code>  -&gt; 203.0.113.10</code> | 这一行表示上一级主题下的子项“203.0.113.10”，重点理解这个子项本身，不要把箭头当成知识点。 |
...
| 第 4 行 | <code>  -&gt; 网卡 eth0</code> | 这一行要理解这些英文词：`eth0` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 第 2 行 | <code>  -&gt; server 203.0.113.10:443</code> | 这一行要理解这些英文词：`server` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 行 | 内容 | 说明 |
|---|---|---|
| 第 1 行 | <code>192.168.1.20:53124 -&gt; 203.0.113.10:443</code> | 这一行表示上一级主题下的子项“192.168.1.20:53124 203.0.113.10:443”，重点理解这个子项本身，不要把箭头当成知识点。 |
...
| 行 | 内容 | 说明 |
|---|---|---|
| 第 1 行 | <code>api.example.com -&gt; A 记录 -&gt; IPv4 地址</code> | 这一行要理解这些英文词：`api.example.com` 是api=应用程序接口；`A` 是英文缩写或固定标识，结合本节上下文记住它代表的组件、命令或状态；`IPv4` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 2 行 | <code>api.example.com -&gt; AAAA 记录 -&gt; IPv6 地址</code> | 这一行要理解这些英文词：`api.example.com` 是api=应用程序接口；`AAAA` 是英文缩写或固定标识，结合本节上下文记住它代表的组件、命令或状态；`IPv6` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 第 2 行 | <code>  -&gt; com</code> | 这一行要理解这些英文词：`com` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 3 行 | <code>    -&gt; example.com</code> | 这一行要理解这些英文词：`example.com` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 第 2 行 | <code>  -&gt; 本机 resolver</code> | 这一行要理解这些英文词：`resolver` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 3 行 | <code>  -&gt; /etc/hosts</code> | 这一行要理解这些英文词：`etc` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题；`hosts` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 4 行 | <code>  -&gt; /etc/resolv.conf 指定的 nameserver</code> | 这一行要理解这些英文词：`etc` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题；`resolv.conf` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题；`nameserver` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 5 行 | <code>  -&gt; recursive resolver</code> | 这一行要理解这些英文词：`recursive resolver` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 6 行 | <code>  -&gt; root / TLD / authoritative</code> | 这一行要理解这些英文词：`root` 是根因或根模块；`TLD` 是英文缩写或固定标识，结合本节上下文记住它代表的组件、命令或状态；`authoritative` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 7 行 | <code>  -&gt; 返回 A / AAAA</code> | 这一行要理解这些英文词：`A` 是英文缩写或固定标识，结合本节上下文记住它代表的组件、命令或状态；`AAAA` 是英文缩写或固定标识，结合本节上下文记住它代表的组件、命令或状态。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 第 2 行 | <code>  &#124; -------- SYN --------------&gt; &#124;</code> | 这一行要理解这些英文词：`SYN` 是英文缩写或固定标识，结合本节上下文记住它代表的组件、命令或状态。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 3 行 | <code>  &#124; &lt;------ SYN + ACK ---------- &#124;</code> | 文本示例行，用来展示输出、目录、流程、错误信息或学习证据中的一条内容。 |
| 第 4 行 | <code>  &#124; -------- ACK --------------&gt; &#124;</code> | 这一行要理解这些英文词：`ACK` 是英文缩写或固定标识，结合本节上下文记住它代表的组件、命令或状态。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 第 4 行 | <code>  -&gt; NGINX / Ingress</code> | 这一行要理解这些英文词：`NGINX` 是高性能 Web 服务器和反向代理，常用于网关、转发和负载均衡；`Ingress` 是Kubernetes 入站流量规则，定义外部请求如何进入集群服务。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 5 行 | <code>  -&gt; App instance 1</code> | 这一行要理解这些英文词：`App instance` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
| 第 6 行 | <code>  -&gt; App instance 2</code> | 这一行要理解这些英文词：`App instance` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
...
| 行 | 内容 | 说明 |
|---|---|---|
| 第 1 行 | <code>Client -&gt; NGINX -&gt; App</code> | 这一行要理解这些英文词：`Client` 是客户端；`NGINX` 是高性能 Web 服务器和反向代理，常用于网关、转发和负载均衡；`App` 是英文关键字，表示本节知识地图里的一个组件、命令、状态或学习主题。`->` 只是知识地图里的层级符号，不是要学习的概念。 |
