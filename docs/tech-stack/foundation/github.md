      ...
      - name: Validate threshold
        ...
        run: |
          ...
          if [[ ! "$value" =~ ^[0-9]{1,2}$ ]]; then
            echo 'threshold must contain one or two digits'
            exit 1
          fi
...
## 平台集成深讲：收到事件，不等于业务已经完成

### 从一次推送到变更事件，先分清两条连接

你的自动化平台想把代码发布加入根因分析时间线，不必让模型反复扫描所有提交。Webhook（事件回调）会在订阅事件出现时，把事件数据发送到你管理的接收地址。它与 Git 推送连接不同：一次推送已成功，后续回调仍可能因接收服务不可达而失败。因此仓库有新提交但时间线上没出现，不能直接判断 GitHub 丢了提交，应该继续查看回调投递和消费状态。

接收端先验证请求来源与内容完整性，再核对事件类型、动作、仓库身份和目标分支。只订阅需要的事件，避免把每条评论都当部署记录。实际部署时间应来自部署完成证据，推送时间只能作为变更候选时间；否则自动根因分析会把尚未发布的代码与线上告警错误关联。

官方建议接收端及时响应，耗时处理放到异步队列；目前文档给出的响应边界为十秒。本课的工程设计是持久化接收记录后再确认，后续消费者补齐检查和部署信息。这个顺序让“接收成功”具有可恢复含义，但仍不等于“业务处理成功”。必须分别监控接收失败、队列积压、处理失败和最终事件写入成功。[Webhook 最佳实践](https://docs.github.com/en/webhooks/using-webhooks/best-practices-for-using-webhooks)

### 重复、遗漏和乱序不能靠一个绿色响应解决

学生问：“既然返回成功，就保证只处理一次了吧？”不保证。接收端可能完成写入后丢失响应，运维又可能手动补投，消费者还可能在外部操作完成后崩溃。投递标识能帮助识别重复，业务层也需要稳定操作标识，比如“仓库、目标环境、部署编号”组合。不要仅以标题相同去重，因为不同提交可能使用相同标题。

同一回调重新投递时，投递标识保持不变；需要把它和处理结果存入持久化记录。若只把已处理标识放在内存集合里，进程重启后就会忘记。去重记录与业务写入若不是一个原子操作，还存在一边成功、另一边失败的窗口。设计时明确这种窗口怎样通过查询现有结果、幂等写入或补偿流程收敛，不要随口承诺跨系统严格一次。

还要纠正一个常见假设：GitHub 不会自动重新投递所有失败回调。需要按官方能力人工重投，或建立受控失败巡检与补投流程。补投前保留失败原因，并设置次数和退避；持续权限错误不应无限重试。遇到事件到达顺序与实际状态不同，消费者可重新读取相关对象的当前状态进行协调，而不是用晚到的旧事件覆盖已完成状态。[失败投递处理](https://docs.github.com/en/webhooks/using-webhooks/handling-failed-webhook-deliveries)

### 接口响应的含义：不存在、不可见和被限制要分开

REST API 是通过标准网络请求读取或修改平台对象的接口。接入前先确定使用的身份、仓库范围、所需对象权限及组织策略。浏览器中你能打开一个私有仓库，不代表后台脚本使用的令牌也能访问；二者可能是不同账号、不同授权范围或不同安装身份。

私有资源返回四百零四时，可能是不存在，也可能是当前身份无权看到；这样可以避免向未授权者泄露资源存在性。先核对仓库名称、接口地址、凭据归属和权限，再考虑资源是否删除。四百零三也不能一概解释为密钥错误，它可能与权限、组织策略或限流有关；应结合响应体和相关响应头判断。排障记录保留状态码、请求标识和时间，不保留完整凭据。[REST API 排障](https://docs.github.com/en/rest/using-the-rest-api/troubleshooting-the-rest-api)

分页又会制造一种隐蔽的数据质量问题：请求成功只代表这一页成功，不代表你拿到了全部记录。统计最近一周的失败部署时，应跟随返回的下一页链接，明确时间边界和过滤条件，并核对重复项。不要凭第一页没有目标对象就宣布“没有失败”。并发更新期间不同页面不是天然一致快照，重要统计要采用稳定时间窗口，必要时重复核对边界记录。

### 限流是容量反馈，不是必须绕过的错误

每个仓库每秒轮询一次，仓库数量增加后会让无变化查询占据大量请求。先问所需数据允许延迟多久，再组合事件通知与低频对账。响应中的剩余额度、重试等待和重置时间是控制请求节奏的证据；不能固定每秒重试，直到平台放行。突发并发或频繁写操作也可能触发另一类限制，剩余主额度看起来充足并不证明可以继续猛发请求。

条件请求能让接收端说明“我已看到这个版本，只有变化才返回完整内容”。保存服务端给出的实体标识，后续请求携带相应条件；收到未修改响应时复用已保存内容，不应把空响应体当成空数据覆盖报表。对认证和响应条件满足要求的未修改请求，官方有不计入主速率限制的说明；这不是所有接口、所有失败或匿名请求的统一豁免。[REST API 最佳实践](https://docs.github.com/en/rest/using-the-rest-api/best-practices-for-using-the-rest-api)

设计容量时分别估算日常对账、故障补投和人工集中发布的峰值。假设两百个仓库每分钟读取一次，就是每小时一万两千次请求，还未计分页和详情；这个算式只表示你的需求，不代表任何账户配额。把有效变更数除以请求数，可以发现大量空轮询。队列变长时先减少非关键扫描，保证真正的发布核验不被统计任务挤占。

### 权限变更也需要版本和回退思路

保护规则、环境审批和自动化身份属于控制配置，不一定全部跟随业务代码提交。一次规则调整可能让所有紧急修复都无法合并，也可能悄悄放开原先受保护的路径。修改前记录现行规则和预期差异，在练习仓库分别验证“应允许”和“应拒绝”两类动作，再逐步应用到目标范围。只有正向测试，无法发现权限过大。

恢复策略应包括重新建立合理规则，而不是简单撤掉全部门禁。若新检查名称与要求的名称不一致，修复映射或恢复旧名称，避免长期取消检查要求。若某个外部检查暂时不可用，紧急通道需要有授权人、时间范围和事后补审。平台本身的可用性由托管方提供，团队仍需准备无法访问平台时的只读资料、已验证产物和暂停发布准则。

### 生产设计题：让告警自动关联最近变更

面试官问：“一百个服务都在 GitHub，如何给告警附上相关发布？”先建立服务、仓库、环境的明确映射，再接收部署事件，保存提交号、部署标识、完成时间和结果。告警触发后只筛选同环境、合理时间窗内的已生效变更，并展示候选与证据，不让模型把相关性直接当根因。回调缺失时用低频对账修复，重投通过幂等处理避免重复卡片。

追问“服务收到重复回调后发了两次回滚指令怎么办”，答案首先是自动分析与高风险执行应分离。回滚必须有独立授权、操作标识、当前版本校验和受控执行记录；重复事件最多刷新建议，不能重复触发写操作。再追问“关联结果漏了一半”，检查分页、权限、服务映射、时间口径、事件投递和消费积压，而不是立刻更换模型。

独立练习使用纸面事件表即可：准备一个成功部署、同一投递的重复事件、一个失败部署和一个晚到的旧事件，预测最终应保留哪个版本、几条记录、哪些需要人工核对。与前面的真实练习仓库实验不同，这里只是离线生产设计推演，不能记为已验证 GitHub 回调服务。保存状态推演、假设和缺失证据，比画一条没有异常分支的自动化箭头更有学习价值。
