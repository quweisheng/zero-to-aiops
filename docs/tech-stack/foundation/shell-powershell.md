...
运维排障第一现场往往不是机器学习模型，而是一串命令：看进程、查端口、读日志、测网络、抓资源。Shell 和 PowerShell 能把这些命令组织成脚本，让排障步骤可重复、可审计、可交给自动化系统执行。

学 AIOps 时，脚本能力是 runbook automation、CI/CD 检查、告警自愈和数据采集的前置能力。
...
- [ ] 我能解释 Shell 文本管道和 PowerShell 对象管道的区别。
- [ ] 我能用 Shell 查看进程、磁盘、内存、端口和日志。
- [ ] 我能用 PowerShell 查看进程、文件、日志和网络连通性。
- [ ] 我能写一个基础健康检查脚本。
- [ ] 我能把脚本输出保存成 Markdown 报告。
- [ ] 我能处理脚本执行权限或执行策略问题。
- [ ] 我能用环境变量保存敏感配置。
- [ ] 我能把常用排障步骤转成 runbook。
...
1. Shell 和 PowerShell 最大区别是什么？
2. 为什么 PowerShell 管道里传的是对象？
3. `set -euo pipefail` 常用于解决什么问题？
4. 如何用 Shell 快速定位端口监听情况？
5. 如何用 PowerShell 测试某个域名端口是否可达？
6. 为什么脚本里不要明文写 token？
7. 如何把人工排障命令转成自动化 runbook？
8. 脚本输出乱码时怎么排查？
9. CI/CD 中常用脚本做哪些检查？
10. 一份好的健康检查脚本应该输出哪些内容？
