...
先读一个完整请求：用户访问域名，入口把流量送到服务，服务找到就绪的 Pod，应用再连接数据库。Pod 是一组协同运行容器的调度单元，Service 是相对稳定的访问入口，Ingress 是 HTTP 入口规则。IaaS 是基础设施即服务，负责虚拟机、网络、存储等底座。先把底座与容器平台的关系画清，再学平台页面上的操作。

第一轮跟 [Kubernetes 精讲](../tech-stack/cloud-native/kubernetes.md) 完成基础实验，回答“对象写在哪里、谁让它生效、在哪里看状态”。第二轮制造一个隔离故障，先预测 Pending（等待调度）、Running（进程运行）和 Ready（可接收业务流量）的区别，再按事件、日志和流量验证。第三轮才做容量、控制面备份、升级和多团队权限。

每天练一道五层题：“是什么 → 请求怎么走 → 为什么这样设计 → 坏了怎么证明 → 生产怎样取舍”。例如 Service 不通，先核对选择器、后端端点、就绪状态和真实监听，再看网络策略，避免背一个固定命令顺序却不知道证据含义。CI/CD 是持续集成与交付或部署，连接代码、镜像、测试、发布和验收，不只是一条构建任务。

30 秒练习：说明你能独立完成的环境与实验，不把岗位年限要求变成自己的履历。3 分钟练习：画一条请求链并讲一个真实实验失败。交给同学复现时，他应能从仓库里的环境、命令、输出和清理步骤完成同样过程。遇到商业平台或现场资源不可得，就明确模拟范围并补文档推理。
